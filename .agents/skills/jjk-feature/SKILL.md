---
name: jjk-feature
description: "Use when you need `jjk-feature` in this repository. Source intent: 全流程开发入口（澄清->规划->实现->审查->验证）：单命令编排，禁止跳阶段"
---
...
目标：避免“解释型对话”被误判为“立即执行型任务”。

规则：

1. 默认模式为 `analysis-only`：仅输出澄清/规划/建议，不进入实现、审查、验证阶段。
2. 只有当用户在**当前轮**明确表达执行意图（如“执行/落地/开始改/直接做”）时，才切换为 `execute`。
3. 若用户仅回复“好的/继续/嗯”等确认词，且未出现执行动词，必须输出 `FEATURE_EXECUTION_INTENT_REQUIRED` 并停在分析态。
4. `analysis-only` 下允许产出设计与规划建议，但禁止触发 `$jjk-imp`、`$jjk-review`、`$jjk-verify` 实施链路。
5. 命中执行意图后，仍需按本命令既有门禁（design 审批、implementation_ready）逐步推进。
