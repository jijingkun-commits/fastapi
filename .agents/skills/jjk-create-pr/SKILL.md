---
name: jjk-create-pr
description: "Use when you need `jjk-create-pr` in this repository. Source intent: PR 交付入口（消费 pr_ready_manifest）：校验任务映射与验收证据后创建/更新 Pull Request"
---
...
1. `$jjk-imp`、`$jjk-imp-ws`：负责产出 `pr_ready_manifest` / `pr_ready_manifest_ws`。
2. `verification-before-completion`：负责“完成前证据”方法（可用时优先）。
3. `requesting-code-review`：负责提交前评审建议（高风险变更建议启用）。
4. `team`（OMX）：多 PR 批量交付时并行处理与汇总。
5. `$jjk-create-pr`：负责映射校验、PR 描述编排、状态回填与交接口径。

约束：

1. 禁止在 `$jjk-create-pr` 重写实现计划语义；只消费 manifest 与计划映射。
2. 禁止创建“无任务归属”的 PR（必须可追溯到 `task_id` 与 `pr_id`）。
3. GitHub 操作优先 `github-mcp-server`，禁止使用 `gh` CLI 作为默认路径。

## 跨 IDE 调用方式

1. Cursor / Claude Code：`$jjk-create-pr`
2. Codex：`$jjk-create-pr`

> 说明：Codex 推荐显式调用 `$jjk-create-pr`。
...
1. 全局共享模板（默认主模板）：
   `/Users/jijingkun/.codex/engineering/templates/jjk_create_pr_templates.md`
...
| 场景 | 推荐命令 |
|---|---|
| 单个任务已完成，准备提 PR | `$jjk-create-pr` ✅ |
| 多个 WS 已完成，批量提 PR | `$jjk-create-pr`（自动 Team）✅ |
| 尚未产出 `pr_ready_manifest` | 先回到 `$jjk-imp` 或 `$jjk-imp-ws` |
...
`$jjk-team-create-pr` 不再作为主入口。
统一由 `$jjk-create-pr` 在批量交付时自动升级 Team 模式。
...
见全局模板：`/Users/jijingkun/.codex/engineering/templates/jjk_create_pr_templates.md`（`输出模板` 段）。
...
`$jjk-imp | $jjk-imp-ws -> $jjk-create-pr -> $jjk-review -> $jjk-verify`
