---
name: jjk-security-audit
description: "Use when you need `jjk-security-audit` in this repository. Source intent: 安全审计入口（消费 review/manifest）：依赖+代码+配置+运行面多层审计并产出可执行修复清单"
---
...
2. Codex：`$jjk-security-audit`

> 说明：Codex 推荐显式调用 `$jjk-security-audit`。
