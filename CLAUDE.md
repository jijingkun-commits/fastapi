...
## Layer1 执行治理（强制）
1. **工作模式默认 `lean`（激进瘦身模式）**：核心是**主动消灭浪费**——优先删除冗余代码、收敛重复逻辑、缩短调用链、消除不必要抽象；**本项目未上线，架构设计质量绝对优先于兼容性与改动量**，根因在结构/依赖/状态归属层面必须 `refactor`，禁止用 `patch` 或兼容层掩盖，修改完代码必须做检查，把当前模块再也用不到的代码删除。
