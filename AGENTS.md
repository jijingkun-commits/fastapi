...
8. 智能体优先：本项目为智能体项目，涉及智能体流程（如 `app/ai/**`）的功能实现时，优先采用流程编排、策略配置、工具抽象与状态管理方案；禁止优先通过硬编码业务分支或固定决策路径实现功能。
9. OpenClaw 对标优先：长期参考项目为 `/Users/jijingkun/bojxAI/bot/openclaw`，涉及智能体架构、流程编排、工具抽象、状态管理与关键实现方案时，优先对齐其设计理念与实现方式；若与本项目约束冲突，需先说明差异再给出落地方案。
...
当任务涉及“修改代码 / 运行测试 / 构建 / 数据迁移”等执行类操作时，必须先执行并回传以下命令结果：
...
1. 若任务已给定目标分支或 worktree，必须与校验结果一致；不一致时立即停止执行，并说明差异。
2. 若任务未给定目标分支或 worktree，先回传校验结果，再继续后续执行。
3. 校验通过后方可执行后续命令；命令优先使用显式路径（如 `pnpm -C web ...`）。
4. 纯问答、纯方案讨论、纯文档阅读类任务可跳过本节。
