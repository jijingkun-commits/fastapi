...
9. **Lean 硬门禁**：热点目录/热点文件必须通过 `lean-guard`；超阈值文件继续净增长、继续新增私有 helper、继续新增嵌套函数时默认阻断，禁止以“后续再治理”替代当前收口，技术细则统一引用 `.cursor/rules/core.mdc` 第 16 条与 `docs/工程规范/lean-guard.md`。
