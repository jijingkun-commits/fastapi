...
## Layer1 执行治理（强制）
1. **工作模式默认 `lean`（激进瘦身模式）**：核心是**主动消灭浪费**——优先删除冗余代码、收敛重复逻辑、缩短调用链、消除不必要抽象；**本项目未上线，架构设计质量绝对优先于兼容性与改动量**，根因在结构/依赖/状态归属层面必须 `refactor`，禁止用 `patch` 或兼容层掩盖，修改完代码必须做检查，把当前模块再也用不到的代码删除，尽量不要写特殊处理代码。
...
4. **变更量约束**：在 `bugfix/refactor` 中若 `新增行数 > 删除行数`，默认视为未完成瘦身，必须说明架构必要性；命中热点目录/热点大文件时，继续净增长默认失败，技术细则统一引用 `.cursor/rules/core.mdc` 第 16 条。
...
7. **证据化交付**：未给出瘦身证据（删除清单、重复收敛、复杂度变化、验证结果）不得宣称 `lean/refactor` 完成。
...
11. **Lean 硬门禁**：热点目录/热点文件必须通过 `lean-guard`；超阈值文件继续净增长、继续新增私有 helper、继续新增嵌套函数时默认阻断，禁止以“后续再治理”替代当前收口，技术细则统一引用 `.cursor/rules/core.mdc` 第 16 条与 `docs/工程规范/lean-guard.md`。
