...
def _extract_card_status_map(state_payload: dict[str, Any]) -> dict[str, str]:
    if not isinstance(state_payload, dict):
        return {}
    card_status = state_payload.get("card_status_map") or state_payload.get("card_status") or {}
...
def _extract_merge_results_map(state_payload: dict[str, Any]) -> dict[str, dict[str, Any]]:
    if not isinstance(state_payload, dict):
        return {}
    merge_results = state_payload.get("merge_results") or {}
    if not isinstance(merge_results, dict):
        return {}
    normalized: dict[str, dict[str, Any]] = {}
    for card_id, payload in merge_results.items():
        normalized_card = _normalize_id(card_id)
        if not normalized_card or not isinstance(payload, dict):
            continue
        normalized[normalized_card] = payload
    return normalized
...
def _resolve_task_state_dir(state_dir: Path, task_key: str) -> Path:
    ...
    if not sanitized_task_key:
        return state_dir
    return state_dir / sanitized_task_key
...
def run_check(
    *,
    ...
    state_payload = load_json(state_file) if state_file.exists() else {}
    status_map = _extract_card_status_map(state_payload)
    merge_results_map = _extract_merge_results_map(state_payload)
    ...
    for card_id in merge_required_cards:
        inline_merge_payload = merge_results_map.get(card_id) or {}
        card_result: dict[str, Any] = {
            "card_id": card_id,
            "merge_result_source": "state_inline",
            "merged": False,
            "merge_commit": "",
            "baseline_visible": False,
        }
        merge_payload: dict[str, Any] = {}
        if isinstance(inline_merge_payload, dict) and inline_merge_payload:
            merge_payload = inline_merge_payload
        else:
    ...
    if missing_merge_result:
        errors.append(f"state.merge_results 缺失: {missing_merge_result}")
