            ...
            <div className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3">
                <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                        ...
                        <span 
                            className={cn(
                                "font-medium text-sm transition-colors line-clamp-1 min-w-0 shrink",
                        ...
                        {todo.due_date && (
                            <span className={cn(
                                ...
                                <span className="hidden sm:inline">{formatFriendlyDate(todo.due_date)}</span>
                            </span>
                        )}
                        ...
                        <Button
                            variant="ghost"
                            size="icon"
                            className="h-7 w-7 sm:h-6 sm:w-6 text-muted-foreground hover:text-primary hover:bg-primary/10 flex-shrink-0 ml-auto"
                    ...
                    {!readonly && isSelected && (
                        <div className="flex items-center gap-1 sm:gap-1.5 mt-2 flex-wrap">
                            ...
                            <Button
                                size="sm"
                                variant="outline"
                                className="h-7 text-xs font-medium px-1.5 sm:px-2.5 text-slate-600 hover:text-slate-900 hover:bg-slate-50 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800"
                                onClick={(e) => {
                                    e.stopPropagation()
                                    onStartEdit(todo)
                                }}
                            ...
                            {!isCompleted && (
                                <Button
                                    size="sm"
                                    variant="outline"
                                    className="h-7 text-xs font-medium px-1.5 sm:px-2.5 text-emerald-600 border-emerald-200 hover:bg-emerald-50 hover:border-emerald-300 dark:text-emerald-400 dark:border-emerald-800 dark:hover:bg-emerald-950/30"
                                    onClick={(e) => {
                                        e.stopPropagation()
                                        onComplete(todo.id)
                                    }}
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    <Button
                                        size="sm"
                                        variant="outline"
                                        className="h-7 text-xs font-medium px-1.5 sm:px-2.5 text-rose-500 border-rose-200 hover:bg-rose-50 hover:border-rose-300 dark:text-rose-400 dark:border-rose-800 dark:hover:bg-rose-950/30"
