...
# SQLAlchemy 原始 SQL 回显默认关闭；需要排障时显式开启。
...
DOCUMENT_MEMORY_ADMIN_MAX_PAGE_SIZE = int(
    os.getenv("DOCUMENT_MEMORY_ADMIN_MAX_PAGE_SIZE", "100")
)
...
RAGFLOW_PAGE_SIZE: int = int(os.getenv("RAGFLOW_PAGE_SIZE", str(RAGFLOW_TOP_K)))
RAGFLOW_VECTOR_WEIGHT: float = float(os.getenv("RAGFLOW_VECTOR_WEIGHT", "0.6"))
RAGFLOW_TIMEOUT_SECONDS: float = float(os.getenv("RAGFLOW_TIMEOUT_SECONDS", "30"))
