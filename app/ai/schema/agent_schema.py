...
def get_agent_schema(agent_id: str) -> Optional[AgentSchema]:
    """获取指定 Agent 的 Schema"""
...
def set_agent_enabled(agent_id: str, enabled: bool) -> bool:
    ...
    借鉴 TypeAgent 的 @config agent 命令
    ...
    if agent_id in AGENT_CAPABILITIES:
        AGENT_CAPABILITIES[agent_id].enabled = enabled
        logger.info(f"Agent {agent_id} {'启用' if enabled else '禁用'}")
        return True
    return False
...
def list_agent_status() -> Dict[str, Dict]:
    ...
    类似 TypeAgent 的 @config agent 表格输出
    """
    result = {}
    for agent_id, schema in AGENT_CAPABILITIES.items():
        result[agent_id] = {
            ...
            "intents_count": len(schema.supported_intents),
            "entities_count": len(schema.supported_entities),
        }
    return result
