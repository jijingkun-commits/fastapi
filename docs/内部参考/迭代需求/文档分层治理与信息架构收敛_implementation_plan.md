...
> 上游设计：`docs/plans/2026-03-10-docs-governance-layering-design.md`
> 对应需求：`docs/内部参考/迭代需求/文档分层治理与信息架构收敛_requirements.md`
> 文档目标：定义 HOW（implementation_tasks、PR 映射、执行合同、实施就绪度），供 `$jjk-imp` 直接承接

## 0. 输入来源清单

- design：`docs/plans/2026-03-10-docs-governance-layering-design.md`
- requirements：`docs/内部参考/迭代需求/文档分层治理与信息架构收敛_requirements.md`
- 既有治理方案：`docs/plans/2026-03-08-doc-single-source-dynamic-governance-design.md`
- 既有实施方案：`docs/内部参考/迭代需求/文档单一真相源与动态融合治理_implementation_plan.md`
...
- `docs/` 的终局只承接稳定真理源；本轮 `Phase 1` 允许 `docs/plans/`、`docs/内部参考/迭代需求/`、`docs/内部参考/任务拆解/` 继续作为迁移期旧过程路径存在，但不再作为主导航入口。
...
implementation_tasks:
  ...
  - task_id: T04
    ...
    acceptance_cmds:
      ...
      - test -z "$(find docs/产品文档 docs/开发文档 docs/API文档 docs/api -type f | rg -v '测试报告|归档备份|防屎山记录手册' | rg '(_v[0-9]+|[-_ ]v[0-9]+|\d{4}-\d{2}-\d{2})' || true)"
      - PYTHON_BIN="$(bash scripts/repo_python.sh)" && "$PYTHON_BIN" scripts/docs_guard.py --strict
  ...
  - task_id: T05
    ...
    file_paths:
      ...
      - docs/开发文档/工作流/文档月度校准清单.md
    symbols:
      - docs_governance_baseline
      - docs_governance_monthly_checklist
    acceptance_cmds:
      - rg -n "目录结构|内容格式|内容同步|迁移收口" docs/开发文档/工作流/文档治理基线清单.md docs/开发文档/工作流/文档月度校准清单.md
  ...
  - task_id: T06
    ...
    file_paths:
      - memory-bank.md
      - docs/内部参考/迭代需求/文档分层治理与信息架构收敛_requirements.md
      - docs/内部参考/迭代需求/文档分层治理与信息架构收敛_implementation_plan.md
      - docs/内部参考/迭代需求/文档分层治理与信息架构收敛_clarify_plan_alignment.json
      - docs/内部参考/迭代需求/文档分层治理与信息架构收敛_planning_temporal_gate.json
    symbols:
    ...
    acceptance_cmds:
      ...
      - PYTHON_BIN="$(bash scripts/repo_python.sh)" && "$PYTHON_BIN" scripts/check_workflow_contract.py --mode clarify_plan --requirements-path docs/内部参考/迭代需求/文档分层治理与信息架构收敛_requirements.md --implementation-path docs/内部参考/迭代需求/文档分层治理与信息架构收敛_implementation_plan.md --output docs/内部参考/迭代需求/文档分层治理与信息架构收敛_clarify_plan_alignment.json
      - PYTHON_BIN="$(bash scripts/repo_python.sh)" && "$PYTHON_BIN" scripts/check_workflow_contract.py --mode planning_temporal_gate --implementation-path docs/内部参考/迭代需求/文档分层治理与信息架构收敛_implementation_plan.md --output docs/内部参考/迭代需求/文档分层治理与信息架构收敛_planning_temporal_gate.json
```
...
tc_task_mapping:
  ...
  - tc_id: TC-DGL-04
    task_id: T05
    pr_id: PR-03
    acceptance_cmd_ref: rg -n "目录结构|内容格式|内容同步|迁移收口" docs/开发文档/工作流/文档治理基线清单.md docs/开发文档/工作流/文档月度校准清单.md
  - tc_id: TC-DGL-05
    task_id: T04
    pr_id: PR-02
    acceptance_cmd_ref: test -z "$(find docs/产品文档 docs/开发文档 docs/API文档 docs/api -type f | rg -v '测试报告|归档备份|防屎山记录手册' | rg '(_v[0-9]+|[-_ ]v[0-9]+|\d{4}-\d{2}-\d{2})' || true)"
```
