...
> 对应需求：`/Users/jijingkun/bojxAI/fastapi/docs/内部参考/迭代需求/全量代码检查与精简治理_requirements.md`
...
- 设计文档：`/Users/jijingkun/bojxAI/fastapi/docs/plans/2026-03-02-full-code-audit-slimming-design.md`
...
1. `docs/plans/2026-03-02-full-code-audit-slimming-design.md`
2. `docs/工程规范/bugfix-minimal-change.md`
...
|---|---|---|---|---|---|---|---|---|
| P1-01 | C01 | 全量热点审计台账；不改业务逻辑 | 扫描执行 -> 输出热点分级 -> 固化台账 | `scripts/ci/check_change_budget.py`（新增）；`docs/plans/2026-03-02-full-code-audit-slimming-design.md` | `module/type/risk/evidence` | 回退到只读报告模式 | `python3 scripts/ci/check_change_budget.py --mode always --strict --diff-range origin/master...HEAD` | design §1.2 |
...
| P5-01 | G01 | 全链路回归与证据归档 | 核心测试 + docs_guard + Gate 卡放行 | `tests/unit/*`；`docs/SUMMARY.md`；`docs/内部参考/迭代需求/全量代码检查与精简治理_implementation_plan.md` | `gate_result/evidence_entry` | Gate 不通过则阻断后续实施 | `venv/bin/python -m pytest -q tests/unit/test_multi_agent_fallback.py tests/unit/test_chat_service_done_payload.py tests/unit/test_data_graph_semantic_guard.py` | design §4.4 |
...
implementation_tasks:
  - task_id: T-01
    ...
    file_paths:
      ...
      - docs/内部参考/迭代需求/全量代码检查与精简治理_implementation_plan.md
    symbols:
  ...
  - task_id: T-09
    ...
    file_paths:
      - docs/SUMMARY.md
      - docs/内部参考/迭代需求/全量代码检查与精简治理_requirements.md
      - docs/内部参考/迭代需求/全量代码检查与精简治理_implementation_plan.md
    symbols:
      - planning_contract
  ...
  cards:
    - card_id: C01
      ...
      acceptance_checks:
        - python3 scripts/ci/check_change_budget.py --mode always --strict --diff-range origin/master...HEAD
        - python3 scripts/check_special_doc_sync.py --strict
      evidence_entry: docs/内部参考/迭代需求/全量代码检查与精简治理_implementation_plan.md
