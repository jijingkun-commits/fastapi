...
| P1-02 | Worker 抢占与状态机 | `pending->processing->succeeded/failed/dead_letter` | `app/services/memory_intent_worker_service.py`（新增） | `attempt_count,lease_until,next_retry_time` | 停 worker，回到只检索 | `venv/bin/python -m pytest tests/unit/test_memory_intent_worker_service.py -q` | design 4.7/4.8 |
