...
- 触发条件: 前置依赖 `G01` 完成
- 输入: `.omc/state/attempts/<card_id>/merge_result.json` + `master` 基线
- 输出: `IG01` 门禁验收结果
...
- 代码锚点（函数/类级）:
  ...
  - `.omc/state/attempts/<card_id>/merge_result.json`
...
1. 验证前置 `G01` 已通过。
2. 执行 `python3 scripts/check_integration_gate.py --task-split-dir "2026-03-01_用户个性化永久记忆与管理能力" --baseline master`。
...
- 最小测试集:
- `cd /Users/jijingkun/bojxAI/fastapi && python3 scripts/check_integration_gate.py --task-split-dir "2026-03-01_用户个性化永久记忆与管理能力" --baseline master`
- 验收标准:
