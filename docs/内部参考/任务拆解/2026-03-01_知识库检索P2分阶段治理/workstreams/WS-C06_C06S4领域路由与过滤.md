...
- 本包目标:
- 根据 query 领域判定构建 metadata_condition
- 路由失败可回退全库检索路径
- 降低跨域误召回风险
...
card_export:
  id: WS-C06
  feature_id: P4-01
  card_key: PP-20260301-KB-RETRIEVAL-P2::WS-C06
  title: C06 S4 领域路由与过滤
  type: parallel
  ...
  mechanism_summary:
    ...
    - 降低跨域误召回风险
  code_anchor_refs:
    - app/ai/tools/ragflow_tool.py::_build_metadata_condition
  example_refs:
