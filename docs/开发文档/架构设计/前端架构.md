...
- 聊天页壳层视觉 single entry owner 固定为 `web/src/app/globals.css` 中被组件实际消费的 `chat-*` 主题 class：`Thread` 只负责页面壳层、滚动区与 footer 输入区接线，加载中的 AI 消息组件（`AssistantMessage` / `AssistantMessageLoading`）承担 `runtime-status` 展示，`ChatInput` 只负责输入区接线，禁止在组件内继续保留第二套大段 inline Tailwind 壳层样式。
...
- `status` 渲染规则（2026-02-15，2026-03-11 收口样式 owner）：
  ...
  - `runtime-status` 测试节点必须挂在 AI 回复加载区域，而不是 `Thread` footer；输入区只负责发送与上传，不承载 AI 过程态展示。
  - `AssistantMessage` / `AssistantMessageLoading` 根据 `phase` 控制动画：`done` 为静态状态点，其他阶段保留 pulse/ping。
