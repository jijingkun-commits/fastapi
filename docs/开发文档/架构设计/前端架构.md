...
- `/admin` 页面通过动态加载渲染 `AdminOverviewCockpit`，首屏固定展示 8 块驾驶舱信息：系统状态、业务请求质量、用户提问活跃度、稳定性、容量与成本、数据新鲜度、模块健康矩阵、实时告警/关键变更流。
- 实时数据链路采用 `SSE 优先 + 10 秒轮询降级`，并通过 `admin-overview-realtime-status` 展示当前连接态；但“连接在线”不等于“当前窗口有业务样本”。
- 前端不再通过 `null`、`--`、`unknown` 推断业务语义，所有卡片统一消费后端显式状态：`ok / no_data / stale / degraded`。
- 趋势数据支持 `1h/24h` 双窗口，并与 `summary` / `stream` 读取同一套分钟聚合事实源，避免“历史曲线有值、当前卡片无语义”的漂移。
- 模块跳转由后端输出 `module_key` contract，前端仅负责 `module_key -> route` 映射，不再通过关键词猜测模块归属。
- 驾驶舱卡片继续采用紧凑间距（`p-4`、`gap-3`、`py-2`），但空态文案需区分“无样本”“过期”“降级”三类场景，避免误导后台判断。
