...
关键行为：
- “模型路由”页签展示 5 类场景路由，其中“主对话”已支持按 `model_routing.default_chat` 可视化配置。
- 管理员更新后端 `model_routing.*` 配置后，前端通过 `getModelRouting()` 重新拉取并即时反映。
- 路由可编辑项统一从“启用中的 chat 模型”下拉中选择，避免配置不可用模型。
