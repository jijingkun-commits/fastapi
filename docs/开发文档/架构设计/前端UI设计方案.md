...
- **导航式侧边栏**: 侧边栏样式以后台导航为唯一基线，统一使用同一套 `app-sidebar-*` tokens 与结构 class；聊天页进一步参考轻量对话产品的历史栏节奏：顶部改成品牌 + 动作图标，保留搜索和“对话管理”入口；左侧大号“新对话”按钮删除，搜索入口收口到头部动作并在头部内联展开；进入多选模式后，原来的大卡片工具条收口为同一语言的紧凑管理条。历史项改成更接近纯文本导航的列表，活动态只保留柔和底色与更多按钮；时间分组继续支持折叠。
