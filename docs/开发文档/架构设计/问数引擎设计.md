...
**设计理念**:
- 指标优先：已有 sql_template 的指标直接复用，不调 LLM
- RAG 增强：未命中指标时，通过 DDL + 文档 + 历史问答构建上下文
...
flowchart TD
    ...
    Analyze -->|"metric_query"| Metric[metric_resolve]
    Analyze -->|"free_query"| Schema[schema_retrieve]
    ...
    Execute -->|"成功"| Done([返回结果])
    Execute -->|"失败且可重试"| Generate
    Execute -->|"失败不可重试"| Clarify
```

### 节点职责

| 节点 | 职责 | LLM 调用 | Token 消耗 |
|------|------|----------|-----------|
| `analyze_data_intent` | 意图分析（metric_query/free_query/clarification） | `get_llm(internal=True)` | ~500 |
| `metric_resolve` | 指标模板匹配，命中则直接生成 SQL | 无（TODO: 向量匹配） | 0 |
| `schema_retrieve` | DDL/文档/历史问答向量检索 | 无（仅 Embedding） | 0 |
| `sql_generate` | 调用 LLM 生成 SQL（Vanna submit_prompt） | `get_llm()` via submit_prompt | ~4000-8000 |
| `sql_safety_check` | sqlglot 解析 + 权限校验 + LIMIT 注入 | 无 | 0 |
| `sql_execute` | 执行 SQL 并格式化结果 | 无 | 0 |
...
### 3.1 当前实现（2026-02-07 已改造）

`metric_resolve` 已从硬编码电商模板改为数据库向量匹配（见 ADR-011）：

```mermaid
flowchart LR
    Intent["意图分析结果"] --> VecSearch["向量检索\nt_metric_definition\n(query_template 非空)"]
    VecSearch -->|"相似度 > 0.5"| Replace["${data_dt}\n参数替换"]
    VecSearch -->|"未命中"| Vanna["回退到 Vanna\nRAG + LLM"]
    Replace --> Safety["安全检查"]
```

**实测匹配效果**（2026-02-07）:
- "存款余额" → DEP_001（0.616）
- "不良贷款率" → LOAN_009（0.636）
- "科技投入" → A002368（0.592）
- "退出系统数量" → A000981（0.631）

**优势**:
- 命中指标时跳过 `schema_retrieve` 和 `sql_generate`，省 1 次 LLM 调用
- Token 消耗从 ~8000 降至 ~750（仅意图分析）
