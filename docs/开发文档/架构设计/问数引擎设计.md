...
4. **Schema 白名单检查**: 仅允许 `ANALYTICS_SCHEMAS` 中配置的 Schema，同时放行 `ALLOWED_METADATA_VIEWS`（information_schema 只读视图）
5. **多语句检测**: 移除字符串字面量和注释后按分号分割，防止 SQL 注入
6. **自动添加 LIMIT**: 通过 `add_limit_if_missing()` 为无 LIMIT 的 SELECT 添加 `LIMIT 1000`

> **权限校验的位置**: 表级/行级/列级权限校验不在 `sql_safety_check` 节点执行，而是在下游 `sql_execute` 节点通过 `app/ai/utils/sql_rewriter.py` 的 `check_and_rewrite_sql(sql, user_id)` 完成。该函数负责表级权限拒绝、行级过滤（RLS WHERE 注入）和列级脱敏。
...
为解决访问控制配置键分裂问题，问数访问控制统一采用 `askdata.*` 主键：
...
3. Schema 黑名单：`askdata.schema_blacklist`（兼容读取历史键 `askdata.schema_whitelist` 与 `data_access.schema_whitelist`）
