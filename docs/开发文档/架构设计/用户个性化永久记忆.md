...
5. 支持灰度开关、降级回退与运维补偿，避免影响主对话链路稳定性。
...
1. 读取开关：`feature.enable_document_memory` + `feature.enable_document_memory_recall`。  
...
| 存储层 | 定位 | 典型内容 |
|---|---|---|
| `t_user_memory`（KV） | 控制层偏好记忆 | 响应语言、风格、稳定人设参数 |
| `t_user_memory_document/chunk` | 长期自由知识沉淀 | 背景事实、长期目标、历史决策、上下文事件 |

### 7.2 灰度迁移建议

1. Phase A：开启 `feature.enable_document_memory` + `flush`，先写不读。  
2. Phase B：开启 `recall` 但小流量灰度，观察注入质量与耗时。  
3. Phase C：开启 `hybrid_search`，配合 embedding 补偿任务。  
4. Phase D：稳定后将 KV 保留为控制层，不再承载自由知识主体。

### 7.3 回滚策略

任一阶段出现异常，可直接关闭以下开关回退，无需回滚表结构：
