...
| SP-013 | streaming_wrapper 上帝函数 | 架构设计 | 中 | 跟踪中 | 2026-02-04 | 2026-02-04 |
...
| SP-017 | get_scene_llm 旧参数兼容映射 | 兼容补丁 | 低 | 跟踪中 | 2026-02-15 | 2026-02-15 |
...
模型场景治理已切换为 `scene_key=模块.函数名`，但历史代码仍存在 `get_scene_llm(scene=default_chat/lightweight/sql_generation)` 调用。一次性删除旧参数会导致存量分支或未同步脚本直接报错。
...
在 `get_scene_llm` 中保留 `scene` 参数兼容层：

1. 当传入 `scene_key` 时，直接走新链路。
2. 当仅传入旧 `scene` 时，映射到预定义 `scene_key` 并记录 warning 日志。
3. 运行时模型解析统一走 `LLMSceneService`，不再读取旧路由回退。

### 为什么不走统一架构

统一架构已经是 `scene_key`，本兼容层仅用于平滑迁移，避免多分支并行期间出现硬切断。

### 潜在风险

1. 旧参数长期保留会让调用点粒度退化，影响场景治理精度。
2. 映射关系是多对一，可能掩盖真实调用点配置缺失。

### 优化方向

1. 完成全仓 `scene_key` 改造后移除 `scene` 参数兼容逻辑。
2. 在 CI 增加规则，禁止新增 `get_scene_llm(scene=...)` 调用。
3. 删除映射后将该条目标记为“已修复”。

### 状态

- 当前状态：**跟踪中**（2026-02-15，兼容映射保留用于平滑迁移，待全仓 scene_key 完成后下线）。
