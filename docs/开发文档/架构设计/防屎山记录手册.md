...
|------|------|------|------|------|----------|----------|
| SP-001 | 消息去重机制 | 数据一致性 | 中 | 已修复 | 2026-01-31 | 2026-03-10 |
| SP-002 | DeepSeek reasoning_content 补丁 | 框架兼容 | 中 | 跟踪中 | 2026-01-31 | 2026-03-10 |
| SP-003 | 异常吞没模式 | 错误处理 | 中 | 可接受 | 2026-01-31 | 2026-03-10 |
| SP-004 | 前端类型断言绕过 | 类型安全 | 中 | 待处理 | 2026-01-31 | 2026-03-09 |
| SP-005 | MinIO URL 编码修复 | 数据处理 | 低 | 可接受 | 2026-01-31 | 2026-01-31 |
| SP-006 | is_intermediate 中间消息标记 | 数据一致性 | 低 | 可接受 | 2026-01-31 | 2026-03-09 |
| SP-007 | SessionStorage 组件通信 | 架构设计 | 中 | 待处理 | 2026-01-31 | 2026-03-09 |
| SP-008 | 默认值兜底逻辑 | 错误处理 | 中 | 待处理 | 2026-01-31 | 2026-03-09 |
| SP-009 | 图片链接自动修复 | 数据处理 | 低 | 可接受 | 2026-01-31 | 2026-03-10 |
| SP-010 | 调试日志硬编码路径 | 代码质量 | - | 已修复 | 2026-01-31 | 2026-02-04 |
| SP-011 | 知识库图片占位符机制 | 架构设计 | 低 | 可接受 | 2026-01-31 | 2026-03-10 |
| SP-012 | 待办卡片乐观更新 | 架构设计 | 低 | 可接受 | 2026-02-02 | 2026-03-09 |
| SP-013 | streaming_wrapper 上帝函数 | 架构设计 | 中 | 跟踪中 | 2026-02-04 | 2026-03-10 |
...
| SP-016 | SSE 状态文案归一化防闪烁 | 交互兼容 | 低 | 已修复 | 2026-02-14 | 2026-03-10 |
...
| SP-020 | Planner 模型主判定失败兜底 | 兼容补丁 | 中 | 跟踪中 | 2026-02-28 | 2026-03-10 |
...
| SP 编号 | 本轮命中文件 | 变更摘要 | 当前结论 |
|------|------|------|------|
| SP-001 | `app/ai/workflow/multi_agent_graph.py` | 预构建 Agent API 从 `create_react_agent` 收口到 `create_agent`，未改消息去重与 replay 去重主链路 | 保持“已修复”，继续观察消息去重副作用 |
| SP-002 | `app/ai/workflow/multi_agent_graph.py` | 本轮仅调整 Agent 构建入口，不新增新的框架兼容补丁或模型分支 | 保持“跟踪中” |
| SP-003 | `app/ai/workflow/multi_agent_graph.py` | 未新增异常吞没点；迁移失败仍通过现有运行态与验证证据暴露 | 保持“可接受” |
| SP-009 | `app/ai/workflow/multi_agent_graph.py` | 知识库图片相关补丁链路未扩大；知识库 Agent 仅切换到 `create_agent` 构建入口 | 保持“可接受” |
| SP-011 | `app/ai/workflow/multi_agent_graph.py` | 图片占位与 `kb_images` 事件链路保持原契约，本轮未新增图片兼容分支 | 保持“可接受” |
| SP-013 | `app/ai/workflow/multi_agent_graph.py` | `streaming_wrapper` 继续保持单出口，迁移只动 Supervisor Agent 工厂，不继续向上帝函数堆新 helper | 保持“跟踪中” |
| SP-016 | `app/ai/workflow/multi_agent_graph.py` | `messages / values / custom` 三路流式契约保持不变，状态文案与事件出口未回退 | 保持“已修复” |
| SP-020 | `app/ai/workflow/multi_agent_graph.py` | Planner 主判定与兜底链路未受本轮迁移影响，未新增关键词判定绕路 | 保持“跟踪中” |
...
| SP-016 | `web/src/hooks/useSSEStream.ts`、`web/src/lib/backend.ts` | `run_id`、停止提示与恢复失败提示统一中文，继续维持 SSE 状态文案归一与防闪烁 | 保持“已修复” |
