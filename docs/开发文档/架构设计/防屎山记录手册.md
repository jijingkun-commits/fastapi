...
|------|------|------|------|------|----------|----------|
| SP-001 | 消息去重机制 | 数据一致性 | 中 | 已修复 | 2026-01-31 | 2026-03-13 |
| SP-002 | DeepSeek reasoning_content 补丁 | 框架兼容 | 中 | 跟踪中 | 2026-01-31 | 2026-03-13 |
| SP-003 | 异常吞没模式 | 错误处理 | 中 | 可接受 | 2026-01-31 | 2026-03-13 |
| SP-004 | 前端类型断言绕过 | 类型安全 | 中 | 待处理 | 2026-01-31 | 2026-03-12 |
| SP-005 | MinIO URL 编码修复 | 数据处理 | 低 | 可接受 | 2026-01-31 | 2026-03-10 |
| SP-006 | is_intermediate 中间消息标记 | 数据一致性 | 低 | 可接受 | 2026-01-31 | 2026-03-12 |
| SP-007 | SessionStorage 组件通信 | 架构设计 | 中 | 待处理 | 2026-01-31 | 2026-03-12 |
| SP-008 | 默认值兜底逻辑 | 错误处理 | 中 | 待处理 | 2026-01-31 | 2026-03-12 |
| SP-009 | 图片链接自动修复 | 数据处理 | 低 | 可接受 | 2026-01-31 | 2026-03-13 |
| SP-010 | 调试日志硬编码路径 | 代码质量 | - | 已修复 | 2026-01-31 | 2026-03-12 |
| SP-011 | 知识库图片占位符机制 | 架构设计 | 低 | 可接受 | 2026-01-31 | 2026-03-13 |
| SP-012 | 待办卡片乐观更新 | 架构设计 | 低 | 可接受 | 2026-02-02 | 2026-03-12 |
| SP-013 | streaming_wrapper 上帝函数 | 架构设计 | 中 | 跟踪中 | 2026-02-04 | 2026-03-13 |
| SP-014 | internal 协议块清洗与 metadata 序列化兜底 | 协议兼容 | 中 | 跟踪中 | 2026-02-08 | 2026-03-12 |
| SP-015 | Vision 路由兼容多模态 Chat 模型 | 架构兼容 | 中 | 已修复 | 2026-02-14 | 2026-03-12 |
| SP-016 | SSE 状态文案归一化防闪烁 | 交互兼容 | 低 | 已修复 | 2026-02-14 | 2026-03-13 |
| SP-017 | get_scene_llm 旧参数兼容映射 | 兼容补丁 | 低 | 已修复 | 2026-02-15 | 2026-02-18 |
| SP-018 | 空结果表切换字段兼容映射 | 兼容补丁 | 中 | 跟踪中 | 2026-02-18 | 2026-03-10 |
| SP-019 | DataGraph 会话帧回收兜底 | 兼容补丁 | 中 | 跟踪中 | 2026-02-18 | 2026-03-13 |
| SP-020 | Planner 模型主判定失败兜底 | 兼容补丁 | 中 | 跟踪中 | 2026-02-28 | 2026-03-13 |

### 2026-03-13 命中回填记录

| SP 编号 | 本轮命中文件 | 变更摘要 | 当前结论 |
|------|------|------|------|
| SP-001 | `app/ai/workflow/multi_agent_graph.py` | data 场景运行态 contract 继续单挂到 `router_result_v2.route_decisions[].data_intent`，没有再并排发明第二套问数结果源 | 保持“已修复”，继续观察消息/状态单真源边界 |
| SP-002 | `app/ai/workflow/multi_agent_graph.py` | 本轮没有把语义 fallback 再塞回聊天编排层，问数判定继续前移到 router/resolver | 保持“跟踪中”，继续推动剩余兼容分支离开编排层 |
| SP-003 | `app/ai/workflow/multi_agent_graph.py` | 问数 reject / clarify 改为结构化 `reason_code + clarify_contract`，未新增新的静默吞错或文案兜底分支 | 保持“可接受” |
| SP-009 | `app/ai/workflow/multi_agent_graph.py` | 本轮触碰主图只是接线问数 contract，不涉及图片修复链路扩张 | 保持“可接受” |
| SP-011 | `app/ai/workflow/multi_agent_graph.py` | 知识库图片/占位兼容链路未被这次问数治理继续放大 | 保持“可接受” |
| SP-013 | `app/ai/workflow/multi_agent_graph.py` | 问数语义识别继续外移到独立 router/resolver，主图只保留 contract 注入与 handoff 接线，未再堆新的语义 helper | 保持“跟踪中”，继续削减主图职责 |
| SP-016 | `app/ai/workflow/multi_agent_graph.py` | 问数缺时间直接落结构化澄清后，用户可见追问与运行态 contract 同口径，减少状态/文案双轨闪烁 | 保持“已修复” |
| SP-019 | `app/ai/workflow/data_graph.py` | `data_graph` 改为消费最终 `data_intent` contract，并把 `llm-shadow` 接成异步旁路对账；没有再新增从文案反推槽位的兜底 | 保持“跟踪中”，继续压缩 DataGraph 内剩余补偿职责 |
| SP-020 | `app/ai/workflow/multi_agent_graph.py` | planner fallback 仍保持最小必要兜底；这次只是让 data.query 在 guard 前继续走 canonical frame，不把 shadow 或单词法信号放回 planner 主判 | 保持“跟踪中” |

### 2026-03-12 命中回填记录

| SP 编号 | 本轮命中文件 | 变更摘要 | 当前结论 |
|------|------|------|------|
| SP-009 | `app/ai/utils/image_fixer.py`、`app/ai/workflow/multi_agent_graph.py` | 删除只被孤岛测试覆盖的 `image_fixer` 旧模块，图片链路继续收敛在现有知识检索/占位 contract，不再保留额外补丁入口 | 保持“可接受”，兼容面继续缩小 |
| SP-011 | `app/ai/events.py`、`app/ai/utils/image_fixer.py`、`app/ai/workflow/multi_agent_graph.py` | 删除知识库图片与事件链路上的未调用包装壳和孤岛模块，保留仍在生产路径上的占位/注入 contract，避免历史兼容分支继续挂空壳 | 保持“可接受”，继续朝单入口收口 |
| SP-013 | `app/ai/workflow/multi_agent_graph.py` | 继续删除 `_ensure_intent_plan_covers_runtime`、`_get_common_tools` 等未调用包装壳，热点大文件仍需后续拆职责，但本轮没有再新增并排 helper | 保持“跟踪中”，持续瘦身 |
| SP-016 | `app/ai/events.py`、`app/ai/workflow/multi_agent_graph.py` | 删除 `emit_confirmation`、`emit_stopped`、`emit_done` 等未调用事件壳，状态事件继续走现有单口径结构，不回退到多套 emitter 语义 | 保持“已修复”，口径继续稳定 |
| SP-015 | `app/core/config.py` | 本轮仅将 `DB_ECHO` 默认值收口为环境变量显式开启，目的是减少 SQLAlchemy 原始 SQL 噪音；未调整 `vision` 路由键、模型过滤范围或多模态兼容合同 | 保持“已修复”，Vision 路由兼容口径不变 |
...
| SP-006  | `app/services/chat_service.py`                                                               | 主链路只做合同收口与补发兜底；历史消息存在时，收口补发按当前轮 `HumanMessage` 切片，避免跨轮污染 | 保持“可接受”           |
...
| SP-004 | `web/src/components/chat/index.tsx`、`web/e2e/helpers/mock-chat-api.cjs` | 运行态状态条改回消息流尾部，并把 E2E mock/认证种子收口到共享 helper；前端仍有少量 runtime 类型断言，但没有新增新的 `any/unknown` 扩散面 | 保持“待处理”，后续继续补齐运行态强类型 |
| SP-012 | `web/src/components/chat/index.tsx` | 本轮只调整状态条挂载位置与消息流布局，不改变待办相关快捷入口与既有乐观更新策略 | 保持“可接受” |
...
### 2026-03-12 命中回填记录（有序内容块）

| SP 编号 | 本轮命中文件 | 变更摘要 | 当前结论 |
|------|------|------|------|
| SP-001 | `web/src/lib/backend.ts`、`web/src/hooks/useSSEStream.ts` | 新增 `display_blocks` 事件后，live 展示直接消费 canonical 快照，不再靠多路临时拼装，重复分发窗口继续缩小 | 保持“已修复”，继续观察 |
| SP-002 | `app/services/chat_service.py` | 本轮只补 `display_blocks` 收口事件，没有继续扩张 DeepSeek 兼容补丁边界 | 保持“跟踪中” |
| SP-003 | `web/src/lib/backend.ts`、`web/src/providers/Thread.tsx` | 未知事件与线程初始化失败仍走显式 warning/降级路径，没有新增静默吞错分支 | 保持“可接受” |
| SP-004 | `web/src/types/message.ts`、`web/src/lib/backend.ts`、`web/src/components/chat/messages/ai.tsx` | `display_blocks` 类型显式化，减少运行态 `unknown` 断言，但旧协议兼容仍未完全退役 | 保持“待处理” |
| SP-006 | `app/repositories/chat_repo.py`、`app/services/chat_service.py` | AI 消息持久化优先收口到 `multimodal content`，未新增第二套中间态消息模型 | 保持“可接受” |
| SP-007 | `web/src/hooks/useSSEStream.ts` | 本轮只改流式展示 owner，没有继续扩张 `sessionStorage` 组件通信范围 | 保持“待处理” |
| SP-008 | `app/core/message_display_blocks.py`、`app/repositories/chat_repo.py`、`app/api/v1/endpoints/chat_api.py` | legacy 回放、未知结果类型与占位符未命中降级集中到编译器，不再散落在前端和仓储双侧兜底 | 保持“待处理” |
| SP-009 | `app/repositories/chat_repo.py` | 删除“未引用图片自动补到正文末尾”的保存层补丁，图片自动修复范围已缩回必要链路 | 保持“可接受” |
| SP-010 | `app/repositories/chat_repo.py` | 本轮没有回引调试硬编码路径或临时日志，继续保持收口后的干净状态 | 保持“已修复” |
| SP-011 | `app/ai/events.py`、`app/core/message_display_blocks.py`、`app/repositories/chat_repo.py`、`web/src/components/chat/messages/assistant-content-blocks.tsx`、`web/src/hooks/useSSEStream.ts` | `[IMG-N]` 已降级为上游锚点语法；live 走 `display_blocks`，history 走 `message.content` canonical blocks，前端不再把 placeholder 当最终展示协议 | 保持“可接受”，兼容面已明显缩小 |
| SP-012 | `web/src/components/chat/messages/ai.tsx` | Todo/SQL/Image/Fallback 统一进入单入口块渲染，没有再长出新的消息卡片并排渲染支线 | 保持“可接受” |
| SP-014 | `app/repositories/chat_repo.py` | multimodal block 序列化与 legacy metadata 兼容继续集中在仓储层，没有扩张新的协议清洗旁路 | 保持“跟踪中” |
| SP-016 | `app/ai/events.py`、`web/src/lib/backend.ts`、`web/src/types/message.ts`、`web/src/hooks/useSSEStream.ts` | 新增 `display_blocks` 事件但未重新分裂 `status/done` 生命周期语义，状态闪烁风险没有回归 | 保持“已修复” |
...
| SP 编号 | 本轮命中文件 | 变更摘要 | 当前结论 |
| ------- | ------------ | -------- | -------- |
| SP-004  | `web/src/components/chat/ChatInput.tsx`、`web/src/components/chat/MultimodalPreview.tsx`、`web/src/lib/multimodal-utils.ts`、`web/src/lib/file-upload-messages.ts` | `.docx/.doc` 上传提示与支持范围改为单独文案模块统一收口，避免继续把“不支持”判断散落到输入框和预览组件；本轮未新增新的 `any/unknown` 扩散面 | 保持“待处理”，后续继续补齐上传 contract 强类型 |
| SP-007  | `web/src/components/chat/ChatInput.tsx`、`web/src/components/chat/MultimodalPreview.tsx`、`web/src/lib/multimodal-utils.ts` | 附件上传与预览链路调整未新增新的 sessionStorage/window-event 通信通道，仍只沿用既有待办选择事件链路 | 保持“待处理”，后续继续把跨组件协同收口到单一 store |
...
| SP-006  | `app/services/chat_service.py`                                                                                                                                      | 中间消息/错误落库链路保持兼容标记，只收敛用户可见错误文本；当存在历史消息时，done 补发前先按当前轮 `HumanMessage` 切片，避免上一轮结果污染当前轮正文               | 保持“可接受”                                               |
...
| SP 编号 | 本轮命中文件 | 变更摘要 | 当前结论 |
|------|------|------|------|
| SP-001 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | 送模前消息装配从主图散点逻辑收口到统一入口，减少消息历史与系统注入在多处分叉拼接的风险 | 保持“已修复”，继续观察上下文账本与历史裁剪的一致性 |
| SP-002 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | reasoning 相关上下文预处理仍由主图保留最小必要兼容，本轮仅抽离送模装配 owner，未扩大 provider 补丁面 | 保持“跟踪中”，继续收窄模型兼容补丁边界 |
| SP-003 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | 新增预算账本与上下文构造模块时未引入静默吞错分支，错误仍由流式主链与调用层显式处理 | 保持“可接受”，继续避免上下文工程层吞异常 |
| SP-009 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | Skill 摘要与系统上下文注入收口后，未把图片链接修复之类展示兼容逻辑带入送模上下文工程 | 保持“可接受”，继续维持展示兼容与送模装配解耦 |
| SP-011 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | 知识库/技能相关送模内容改走统一摘要装配入口，未把图片占位符等展示补丁扩散到主图上下文拼装 | 保持“可接受”，继续维持知识注入与展示补丁边界 |
| SP-013 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | `streaming_wrapper` 继续瘦身，送模前上下文工程与 token 账本下沉到独立模块，主图保留编排与运行态职责 | 保持“跟踪中”，后续继续压缩主图剩余预处理分支 |
| SP-016 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | 本轮只调整送模前上下文构造，不改变 SSE 状态文案归一化链路，避免上下文重构误伤前端状态体验 | 保持“已修复”，继续观察流式状态一致性 |
| SP-020 | `app/ai/context_engineering.py`、`app/ai/workflow/multi_agent_graph.py` | planner 主判定失败兜底仍保持最小必要范围，送模前上下文不再通过主图散落补料或把结果态信息硬塞回系统上下文 | 保持“跟踪中”，继续缩小 planner fallback 与上下文工程的耦合 |
...
> **注意**：`chat_graph.py` 单智能体模式已废弃（2026-01-31），系统默认使用多智能体模式。历史备份已迁到 `workdocs/归档/专题/架构历史/单智能体模式备份.md`
...
| 文件 | 作用 |
|------|------|
| `app/ai/tools/ragflow_tool.py` | 继续生成 `[IMG-N]` 占位符与 `kb_images` 映射，作为上游锚点语法 |
| `app/core/message_display_blocks.py` | 把 `final_text + kb_images + result_events` 编译成 ordered content blocks |
| `app/repositories/chat_repo.py` | 持久化 AI 消息时优先落 `multimodal` canonical blocks |
| `app/api/v1/endpoints/chat_api.py` | 历史消息优先返回 canonical blocks，legacy 消息在接口层兼容编译 |
| `app/services/chat_service.py` | 在 stream / resume 收口阶段发送 `display_blocks` 展示快照 |
| `app/ai/events.py` | 定义 `display_blocks` 与 `kb_images` 事件载荷 |
| `web/src/hooks/useSSEStream.ts` | 接收 `display_blocks` 后直接覆盖当前 AI message 的展示内容 |
| `web/src/components/chat/messages/assistant-content-blocks.tsx` | 单入口逐块渲染 markdown/image/sql/todo/fallback |
| `web/src/components/chat/messages/ai.tsx` | 优先消费 canonical blocks，不再现场编译 placeholder |
...
3. 将映射嵌入工具输出：<!--KB_IMAGES:{"0":"url0","1":"url1"}-->
   ↓
4. 后端收口 `final_text + kb_images + result_events`，编译出 ordered content blocks
   ↓
5. live 阶段通过 SSE 发送 `display_blocks`，history 阶段通过 `message.content(blocks)` 回放
   ↓
6. 前端按块渲染；`[IMG-N]` 只保留为上游锚点语法，不再直接参与最终 UI 协议
```
...
flowchart LR
    ...
    subgraph Backend [后端处理]
        B1["收口最终文本 / kb_images / result_events"] --> B2["compile_message_display_blocks"]
        B2 --> B3["SSE: display_blocks"]
        B2 --> B4["DB: content=blocks[]"]
    end

    subgraph Frontend [前端渲染]
        F1["接收 display_blocks"] --> F2["更新当前 AI message.content"]
        F2 --> F3["AssistantContentBlocks\n逐块渲染"]
    end
...
1. **上游锚点仍存在**：`[IMG-N]` 仍是模型可见语法，生成侧如果错位仍会影响块编译结果
2. **legacy 历史消息仍需兼容**：旧消息刷新时仍要在接口层做一次兼容编译
3. **SSE / history 双入口需同口径维护**：live `display_blocks` 与 history `content(blocks)` 都必须走同一编译规则

### 优化方向

1. **继续缩窄上游占位符语法**：未来评估把 `[IMG-N]` 升级成更显式的结构化锚点 contract
2. **淘汰 legacy 回放编译**：待历史存量完成迁移后，删除字符串消息到 blocks 的兼容入口
3. **保持单编译 owner**：禁止再回到“前端猜结构、仓储补图片、接口再兜底”的三路并行

### 修复记录（2026-02-15）

**状态：已显著收口（2026-03-12 更新）**

- `[IMG-N]` 仍保留，但只作为上游锚点语法，不再直接决定前端最终展示。
- live 展示统一走 SSE `display_blocks`，history 统一走 `message.content` canonical blocks。
- 前端 placeholder 替换器已退役，`AssistantMessage` 只做“有块渲块、无块渲纯文本”。
- 仓储层已删除“保存前替换占位符”和“未引用图片自动补到末尾”的旧补丁。
