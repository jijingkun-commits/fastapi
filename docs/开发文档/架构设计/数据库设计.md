...
| `t_exam_generation_job` | `app/models/exam_generation_job.py` | AI 出题任务与历史记录唯一真理源（2026-03 新增） |
...
| `t_runtime_metric_bucket_minute` | `app/models/runtime_metric_bucket.py` | 管理后台总览分钟聚合唯一事实源 |
...
> 用于记录单次流式会话（run）的生命周期状态，支撑取消、恢复与审计。P0 多会话并发口径下，跨刷新 / 跨 worker 的 active run 真理源固定为该表。
...
| `last_activity_at` | TIMESTAMP | 最近活动时间，用于 active run 恢复排序与 stale hint |
...
- `idx_chat_run_user_status_updated` on (`user_id`, `status`, `updated_at DESC`)
...
> 用于承载文档记忆的分块检索索引，支持 `memory_search -> chunk_text/citation 注入` 单阶段召回。
...
| `description` | TEXT | 描述 |
| `status` | ENUM | 状态: todo / in_progress / done / cancelled（完成态仅由 `status='done'` 表示） |
...
### `t_runtime_metric_bucket_minute` - 总览分钟聚合读模型

> 当前总览 V2 的唯一事实源。`summary`、`trends`、`stream` 与运行态写入均围绕该表收口，禁止再引入展示快照双写。

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | SERIAL | 主键 |
| `bucket_minute` | TIMESTAMPTZ | 分钟桶时间，唯一约束的一部分 |
| `scope` | VARCHAR(32) | 统计范围：`all_business / user_question / admin_operation` |
| `module_key` | VARCHAR(64) | 模块标识 |
| `request_count` | INTEGER | 请求数 |
| `success_count` | INTEGER | 成功数 |
| `error_4xx_count` | INTEGER | 4xx 数 |
| `error_5xx_count` | INTEGER | 5xx 数 |
| `latency_histogram` | JSONB | 延迟分桶或可计算 P95 的聚合结构 |
| `cost_total` | NUMERIC | 该分钟累计成本 |
| `last_event_at` | TIMESTAMPTZ | 最后一条事件时间 |
| `created_at` | TIMESTAMPTZ | 创建时间 |
| `updated_at` | TIMESTAMPTZ | 更新时间 |

**约束与索引**：
- 唯一约束：`(bucket_minute, scope, module_key)`
- 索引：`(scope, bucket_minute)`
- 索引：`(module_key, bucket_minute)`
- 索引：`last_event_at`
...
| `tool_contract` | JSONB | version 级工具授权契约（Skill-First Tool Gating） |
...
| `t_ops_metric_snapshot_minute` | ❌ 已删除 | 总览 V1 展示快照表，造成分钟桶与快照双真源，已由 Alembic 正式迁移删除 |

> **重要**：指标定义统一使用 `t_metric_definition` 表；总览运行态统一使用 `t_runtime_metric_bucket_minute`，废弃表已通过正式迁移删除。
...
> 说明：Alembic 主要用于发布链路（测试环境/生产环境）的版本化迁移。日常开发阶段优先使用 `bash scripts/db/run_dev_migration.sh`；需要进入发布链路时，统一使用 `bash scripts/db/run_release_migration.sh --message "<message>"`。
...
```bash
# 生成迁移脚本（基于模型变更自动检测）
bash scripts/db/run_release_migration.sh --message "add is_active to user" --skip-upgrade
...
**脚本入口**: `bash scripts/db/run_dev_migration.sh`（底层实现：`scripts/db/sync_database.py`）
...
flowchart LR
    A[修改模型] --> B{正式环境?}
    B -->|是| C[bash scripts/db/run_release_migration.sh --message "<message>" --skip-upgrade]
    C --> D[review 迁移脚本]
    D --> E[bash scripts/db/run_release_migration.sh --upgrade-only]
    B -->|否| F[bash scripts/db/run_dev_migration.sh]
```

| 环境 | 推荐方案 | 说明 |
|------|----------|------|
| 生产 | Alembic | 可追溯、可回滚、版本化管理 |
| 开发 | run_dev_migration.sh | 快速迭代、统一解释器与同步入口 |
| CI/CD | run_dev_migration.sh | 自动检测并修复差异 |
...
### `t_exam_generation_job` - AI 出题任务表

| 字段 | 类型 | 说明 |
|---|---|---|
| `id` | BIGINT | 主键 |
| `user_id` | BIGINT | 创建任务的管理员 ID |
| `title` | VARCHAR | 试卷标题 |
| `status` | VARCHAR | `queued/running/succeeded/failed` |
| `dataset_ids` | JSON | 本次选择的数据集列表 |
| `request_snapshot` | JSON | 提交时的模板与参数快照 |
| `result_payload` | JSON | 任务唯一结果 canonical |
| `asset_id` | BIGINT | 成功导出的 `t_chat_assets.id` |
| `minio_object_key` | VARCHAR | PDF 在 MinIO 中的对象路径 |
| `error_message` | TEXT | 失败原因 |
...
> 说明：`t_exam_generation_job` 是后台 AI 出题功能的唯一任务真理源；历史记录页只读本表 `result_payload` 与关联 `t_chat_assets`，不回读聊天线程。
