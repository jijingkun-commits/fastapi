...
- 迁移目标：
  ...
  - 初始化 admin/analyst/user 默认权限规则
- 适用场景：老环境缺少问数权限字段或权限表，导致权限重写链路无法按角色/机构生效

### 增量迁移说明（2026-02-16）

- Alembic 迁移版本：`20260216_0006`
- 迁移目标：
  - 在 `t_user` 新增 `data_role` 字段（默认 `staff`）
  - 回填历史用户 `data_role`，保证无 NULL
  - 保留 `role` 作为系统角色兼容入口
- 适用场景：双角色权限模型切换阶段，避免历史账号缺失数据角色导致权限链路不可判定
...
|------|------|------|
| `role` | VARCHAR(50) | 系统角色兼容字段（sys_role）: admin / analyst / user |
| `data_role` | VARCHAR(50) | 数据角色（data_role）: head_president / department_gm / department_vgm / staff |
| `org_code` | VARCHAR(100) | 机构代码 |
| `org_name` | VARCHAR(200) | 机构名称 |
| `dept_code` | VARCHAR(100) | 部门代码 |
| `dept_name` | VARCHAR(200) | 部门名称 |
