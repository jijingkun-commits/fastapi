...
| `t_user_memory` | `app/models/user_memory.py` | 用户跨会话偏好记忆 |
| `t_user_memory_document` | `app/models/document_memory.py` | 文档化永久记忆主表（长期/日常） |
| `t_user_memory_chunk` | `app/models/document_memory.py` | 文档记忆分块检索表（FTS + 向量） |
| `t_user_memory_intent_job` | `app/models/user_memory_intent_job.py` | 记忆判定异步任务队列（主链路入队） |
...
> 用于存储非 KV 形态的用户长期记忆文档，作为永久记忆主真相源。

| 字段 | 类型 | 说明 |
|------|------|------|
| `id` | BIGSERIAL | 主键 |
| `user_id` | INTEGER | 用户 ID（强隔离） |
| `doc_kind` | VARCHAR(32) | 文档类型：`long_term` / `daily` / `session` |
| `doc_key` | VARCHAR(128) | 文档键（如 `MEMORY`、`2026-02-28`） |
...
> 用于承载聊天主链路入队的记忆判定任务，消费端通过状态机推进处理。
