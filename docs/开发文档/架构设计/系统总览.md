...
graph TB
    ...
    subgraph AICore
        Supervisor[Supervisor Agent 路由与简单任务]
    ...
    subgraph Storage
        ChatDB[chat_db 系统用户配置]
        DataDB[data_db 业务与数仓]
        MinIO[MinIO 资产存储]
    end
    ...
    Supervisor --> DataGraph
    Supervisor --> TodoGraph
    Supervisor --> Tools
