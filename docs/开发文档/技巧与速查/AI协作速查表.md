...
| 本地提交并收口主干 | `/jjk-commit` | 已验收后提交当前分支，并通过 shared delivery engine 收口到主工作区 `master` |
...
1. 基础观测、期望比对、解释器解析、测试执行、统计汇总分步跑，禁止拼成长 `&&` / `;` 链。
2. 任一步失败只重跑当前步；长任务启动后只轮询/读结果，不重复启动。
3. 输出被截断时优先拆短当前步或分段读取，禁止为了看尾部结果整串重跑。
