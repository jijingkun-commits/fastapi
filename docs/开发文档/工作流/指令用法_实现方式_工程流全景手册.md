...
> 版本：`v1.0`  
> 更新时间：`2026-03-06`  
> 适用范围：本仓库 `jjk-*` 指令链 + 通用 `/ask /plan /do` 指令。
...
3. 并行链路已具备可执行契约：`/jjk-vkplan -> /jjk-vktodo(create-only) -> /jjk-cardrun(loop) -> /jjk-imp-ws`，核心价值是可追溯、可回放、可阻断。
...
| 运行镜像 | `.claude/commands/*.md`、`~/.codex/prompts/*.md` | 不改语义，仅做跨 IDE 运行镜像 |
...
| Codex | 使用 `/prompts:jjk-xxx`（对应 `~/.codex/prompts/*.md`） |
...
|---|---|---|---|---|
| 发散 | `/ask` | 模糊需求发散，形成单一推荐方向 | 用户问题与约束 | 可选 `brainstorm.md` 探索快照 |
...
```text
/ask(可选) -> /jjk-clarify -> /jjk-plan -> (/jjk-imp | /jjk-wtimp | /jjk-vkplan)
...
```mermaid
flowchart TD
  A["想法/需求输入"] --> B["/ask (可选发散)"]
  A --> C["/jjk-clarify (冻结设计+PRD-Lite)"]
  B --> C
...
| 收口点 | 必须动作 | 阻断条件 |
|---|---|---|
| `vkplan -> cardrun` | `check_plan_vk_coverage.py` 通过 | 合同缺口、task_id 缺失、execution_contract 不一致 |
...
1. **何时用**：需求刚出现、约束不全、要先看 2~3 个方向。
2. **核心动作**：输出单一推荐方向，不做落地实施。
3. **强门禁**：候选方向少于 2 个 -> `ASK_BRAINSTORM_INSUFFICIENT`；无单一结论 -> `ASK_DECISION_MISSING`。
4. **下游**：必须收敛到 `/jjk-clarify`，禁止直跳 `/jjk-imp` 或 `/jjk-vkplan`。
...
4. **强校验**：必须运行 `scripts/check_clarify_plan_alignment.py`，否则不得下游。
...
3. **强校验**：必须跑 `check_plan_vk_coverage.py`，并确保 `clarify_plan_alignment.ok=true`。
...
| 脚本 | 主要命令 | 作用 |
|---|---|---|
| `scripts/check_clarify_plan_alignment.py` | `/jjk-plan` | 校验 Clarify -> Plan 桥接完整性 |
| `scripts/check_plan_vk_coverage.py` | `/jjk-vkplan`、`/jjk-cardrun` | 校验计划到卡片消费覆盖 |
