...
| 规划 | `/jjk-plan` | 输出实施计划与 UAT 用例 | 已审批 `requirements.md` + `design.md` | `implementation_plan.md` + `uat_cases.md` |
...
| 审查 | `/jjk-review` | 审查需求/方案/计划/收口与迁移一致性 | PR/diff/manifest + 真理源 | `review_report_<topic>.md` |
| 测试 | `/jjk-test` | 可追溯测试矩阵执行 | review/manifest/plan | 测试报告 + 测试资产回填 |
| 验收 | `/jjk-verify` | 消费既有合同做最终验收 | requirements/design/plan/uat/evidence | 验收报告（PASS/WARN/FAIL） |
...
| Plan | `implementation_plan.md` + `uat_cases.md` | 必须同时产出施工单与验收单 |
...
2. **必备输出**：`<topic>_implementation_plan.md` + `<topic>_uat_cases.md`。
3. **关键要求**：
   - 每个 task 都能回溯到需求与方案；
...
5. **结果**：输出 `PASS/WARN/FAIL` 与 `verify_report`。
