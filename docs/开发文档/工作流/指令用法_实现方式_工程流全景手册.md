...
| 文档门禁 | `/jjk-api-doc-sync` | 输出 API/Schema/Route 文档同步清单并阻断漂移 | 变更文件范围 / 契约变更说明 | 文档同步清单 + `sync_status` |
...
| 本地合并收口 | `/jjk-commit` | 提交当前分支并合并到主工作区 `master` | 验收证据 + commit message | `commit_sha` + `master` 新 HEAD |
...
# 兼容旧习惯时，`/ask` 会在当前会话内立即并入 `/jjk-clarify`
 -> /jjk-review -> /jjk-test -> /jjk-verify -> (/jjk-create-pr | /jjk-commit)
```

治理前置补充：

1. 任何涉及模块边界、依赖方向、状态归属、错误处理责任的改动，都应在 `/jjk-plan` 或 `/jjk-imp` 前插入 `/jjk-arch-gate`。
2. 命中路由、schema、DTO、接口语义、测试案例或 `docs/API文档/接口文档.md` 变更时，都应在实现前插入 `/jjk-api-doc-sync`。
...
6. **产品运行时 Skill 专项**：命中 `skill_service.py`、`skill_admin_api.py`、`user_skill_api.py`、`agent_skill.py`、Skill runtime 测试、部署/退役脚本等 DB-only 主链文件时，必须按 `.cursor/rules/doc_sync.mdc` 的“产品运行时 Skill 专项映射（强制）”同步产品/API/内部机制/配置/部署/测试文档，并显式保持“本地 `SKILL.md` / 导入脚本已退役”口径，禁止只更测试报告或接口文档。
...
**结论先行**：`/jjk-cardrun` 不是“再来一个实现命令”，而是并行计划落卡后的**串行编排总控**。它的职责是把 `vk_cards.json` 中的卡片顺序、`_active_task.json` 中的活跃上下文、`/jjk-wtimp` 的单卡隔离执行能力，以及 `wt-flow.sh` 的 `verify/merge` 原子动作，收敛为一条唯一、可阻断、可回放、可审计的主链。

| 维度 | 说明 |
|---|---|
| 何时用 | 已完成 `/jjk-vkplan` 拆解并通过 `/jjk-vktodo` 建卡，准备进入“逐卡推进”阶段 |
| 主职责 | 单活卡调度、续跑恢复、执行器分派、`verify -> merge -> done` 收口 |
| 上游依赖 | `parallel_plan.md`、`workstreams/WS-*.md`、`vk_cards.json`、`_active_task.json` |
...
| 类别 | 文件 / 字段 | 作用 | 状态归属 |
|---|---|---|---|
| 执行契约 | `docs/内部参考/任务拆解/<task_split_dir>/vk_cards.json` | 定义 `execution_mode`、`card_order`、`cards`、`task_to_pr_mapping` | 并行计划产物，供 `cardrun` 只读消费 |
...
| 运行态状态 | `.state/<task_key>/task-runner-state.json` | 记录当前卡、状态流转、执行尝试 | `kernel + wt-flow` 维护 |
...
| 阶段 | 机制 | 关键门禁 | 实现锚点 |
|---|---|---|---|
| 1. 合同预检 | 读取 `vk_cards.json` 校验 `execution_mode`、`card_order`、`cards`、`task_to_pr_mapping` 是否完整可解析 | 非 `serial`、空 `card_order`、卡片/PR 映射不完整直接阻断 | `scripts/coder4/coder4_bootstrap_kernel.py` |
...
| 命令 / 组件 | 负责什么 | 不负责什么 |
|---|---|---|
| `/jjk-vkplan` | 把计划拆成 `parallel_plan.md`、`WS-*.md`、`vk_cards.json` | 不负责执行卡片 |
| `/jjk-vktodo` | 把 `vk_cards.json` 幂等落到 Vibe Kanban | 不负责推进状态、不负责串行执行 |
...
1. **目标**：把当前 worktree 上已验证的改动提交，并合并到主工作区 `master`。
2. **硬门禁**：当前就在 `master`、缺少 commit message、缺少验证证据、当前/主工作区不干净时直接阻断。
3. **合并策略**：先定位 `master` worktree，再执行本地 merge；默认优先快进，禁止用强推或跳过校验掩盖问题。
4. **产出**：`commit_sha`、合并后 `master` HEAD、验证证据摘要、是否建议执行 `/jjk-deleteworktree`。
...
**结论先行**：本节说的“状态文件”，只统计会被 `jjk-*` 主工程流脚本**直接读写**、并参与“选任务 / 选卡 / 去重 / 验证 / 合并 / 审计”的文件。`vk_cards.json`、`parallel_plan.md`、`WS-*.md` 这类**合同/计划文件**不算运行态状态文件；`review_report`、`pr_ready_manifest` 这类**交付工件**也不算。
...
| 合同文件 | 否 | `vk_cards.json`、`parallel_plan.md`、`WS-*.md` | 它们定义任务，不记录运行态 |
...
```mermaid
flowchart LR
    A["$jjk-vkplan / scripts/set_active_task.py"] --> B["docs/内部参考/任务拆解/_active_task.json"]
    A --> C["docs/内部参考/任务拆解/<task_split_dir>/_active_task.json"]
    D["外部/操作步骤写入 scope request"] --> E[".state/coder4_scope_request.json"]
    E --> F["scripts/coder4/coder4_scope_guard.py"]
    F --> C
    G["scripts/coder4/coder4_bootstrap_kernel.py"] --> H[".state/<task_key>/task-runner-state.json"]
    G --> I[".state/<task_key>/task-ledger.jsonl"]
    G --> J[".state/<task_key>/coder4-idempotency.json"]
    G -. "flock" .-> K[".state/<task_key>/coder4-run.lock"]
    L["scripts/coder4/wt-flow.sh"] --> H
    L --> M[".state/<task_key>/active-session-<session_id>.json"]
    N["check_integration_gate / vk_sync / contract checks"] --> H
    N --> C
```
...
sequenceDiagram
    ...
    Note over RootA,State: 读路径上通常是：先通过 active 找到 task_key/task_split_dir，<br/>再进入 .state/<task_key> 读取 state / ledger / session / idempotency
```
...
**结论先行**：读链路和写链路不同，它不是“先读某一个大文件就够了”，而是先定位 active，再据此推导 `task_split_dir / task_key`，随后才去解析 `vk_cards.json`、`task-runner-state.json`、`active-session-<session_id>.json` 等具体文件。不同脚本读的终点不同，但入口几乎都从 `_active_task.json` 开始。
...
sequenceDiagram
    ...
    alt kernel / vk_sync 读链
        ...
        K->>State: 解析 .state/<task_key>/task-runner-state.json
...
1. 大多数脚本不是直接“盲读 `.state`”，而是先通过 `_active_task.json` 找到 `task_key/task_split_dir`；
...
|---|---|---|---|---|---|
| `docs/内部参考/任务拆解/_active_task.json` | 根索引 | 给脚本一个默认“当前任务指针” | `scripts/set_active_task.py` | `coder4_bootstrap_kernel.py`、`coder4_vk_sync.py`、其他默认解析逻辑 | 不建议 |
| `docs/内部参考/任务拆解/<task_split_dir>/_active_task.json` | 任务级真理源 | 锁定当前任务的 `task_key/task_split_dir/project_id` 等上下文 | `scripts/set_active_task.py` | `jjk-vkplan`、`jjk-vktodo`、`jjk-cardrun`、`wt-flow.sh`、`coder4_bootstrap_kernel.py`、`coder4_vk_sync.py` | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json` | 主运行态 | 记录卡片状态机、gate、merge 结果 | `coder4_bootstrap_kernel.py`、`wt-flow.sh` | `wt-flow.sh`、`check_integration_gate.py`、`coder4_bootstrap_kernel.py`、`coder4_vk_sync.py` | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-ledger.jsonl` | 事件账本 | 追加记录每轮推进证据 | `coder4_bootstrap_kernel.py` | 审计/排障、`check_workflow_contract.py` 真理源识别 | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/active-session-<session_id>.json` | 会话态 | 记录当前 worktree 会话位置 | `wt-flow.sh create` | `wt-flow.sh merge/status/cleanup`、会话解析逻辑 | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/coder4-idempotency.json` | 运行控制 | 防止短时间重复触发同一轮 | `coder4_bootstrap_kernel.py` | `coder4_bootstrap_kernel.py` | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/coder4-run.lock` | 运行控制 | 保证同一任务同一时刻只有一个 kernel 轮次持锁执行 | `coder4_bootstrap_kernel.py` | `coder4_bootstrap_kernel.py` | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/coder4_scope_request.json` | 可选切换态 | 请求把 active scope 切到某个任务 | 外部编排器/操作步骤写入，`set_active_task.py` 回写 applied 字段 | `coder4_scope_guard.py`、`set_active_task.py` | 不建议 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json.lock` | 辅助锁文件 | `task-runner-state.json` 的文件锁副产物 | `coder4_bootstrap_kernel.py` | 锁机制自身、`check_workflow_contract.py` 识别 | 禁止 |
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json.bak` | 恢复文件 | 状态写入失败时用于恢复 | `coder4_bootstrap_kernel.py` | `load_local_state()` 恢复逻辑 | 禁止 |

### 6.4 两个 `_active_task.json`：名字一样，职责不一样

| 文件 | 本质 | 核心差异 |
|---|---|---|
| `docs/内部参考/任务拆解/_active_task.json` | 根索引 | 是“默认入口指针”，方便脚本在未显式传路径时找到当前任务 |
| `docs/内部参考/任务拆解/<task_split_dir>/_active_task.json` | 任务级真理源 | 是当前任务的正式上下文，真正决定 `task_key/task_split_dir/project_id` |

#### 6.4.1 任务级 `_active_task.json`

**作用**：回答“当前到底是哪一个任务在跑”。它是串起 `vk_cards.json`、`.state/<task_key>`、`wt-flow.sh`、`coder4_bootstrap_kernel.py` 的桥梁文件。
...
2. `/jjk-vktodo`：校验 `task_key/task_split_dir` 与 `vk_cards.json` 一致；
...
| `task_split_dir` | 任务拆解目录名 | `set_active_task.py` | 反向定位 `vk_cards.json`、`workstreams/`、`.state/` |
| `task_key` | 任务主键 / 追溯主键 | `set_active_task.py`（从 `vk_cards.json.task_key` 提取） | 生成 `.state/<task_key>` 路径、构建分支与会话名 |
| `execution_mode` | 执行模式，当前主链要求 `serial` | `set_active_task.py`（从 `vk_cards.json` 读取） | kernel/cardrun 判断是否允许串行推进 |
| `single_active_card` | 是否只允许单活卡 | `set_active_task.py` | scope guard / kernel 保证单卡推进纪律 |
| `auto_done_policy` | 自动完成策略 | `set_active_task.py`（优先 `vk_cards.json.auto_done_policy`） | 决定卡片完成策略口径 |
| `preflight_required` | 预检卡 ID，默认常见为 `C00` | `set_active_task.py`（优先 `vk_cards.json.preflight.card_id`） | kernel 判断是否允许进入正式推进 |
...
```json
{
  ...
  "status_source_of_truth": "/Users/jijingkun/bojxAI/fastapi/docs/内部参考/任务拆解/2026-03-06_cardrun_wtimp_executor/preflight_status.json",
...
```json
{
  ...
  "dispatch_executor": "wtimp",
  "dispatch_executor_mode": "cardrun_dispatch",
  "active_task_path": "/Users/jijingkun/bojxAI/fastapi/docs/内部参考/任务拆解/2026-03-06_cardrun_wtimp_executor/_active_task.json"
}
```
...
| `task_key` | 当前状态文件所属任务主键 | kernel / 上游初始化 | 用于确认 `.state/<task_key>` 与 active 一致 |
...
| 项 | 说明 |
|---|---|
| 路径 | `.state/<task_key>/coder4-run.lock` |
...
**默认路径**：`docs/内部参考/任务拆解/<task_split_dir>/.state/coder4_scope_request.json`。
...
| `.state/<task_key>/.session-state.lock` | 会话文件写入锁目录 | `wt-flow.sh` | 这是目录不是 JSON 文件，只负责会话互斥 |
...
补充约定：执行 `scripts/coder4/wt-flow.sh status` 时，应优先把 `active_task_file / active_task_key / task_state_root` 与同 split_dir 下的 `STALE_STATE_CANDIDATES` 一并看出来；不要只盯某个历史 `.state/<task_key>/` 目录做人肉判断。
...
```text
/jjk-clarify -> /jjk-plan -> /jjk-imp -> /jjk-review -> /jjk-test -> /jjk-verify -> (/jjk-create-pr | /jjk-commit)
```
...
```text
/jjk-debug -> /jjk-review -> /jjk-test -> /jjk-verify -> (/jjk-create-pr | /jjk-commit)
```
