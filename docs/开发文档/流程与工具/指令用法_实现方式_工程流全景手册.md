...
3. **关键要求**：
   - 每个 task 都能回溯到需求与方案；
   - task 明细要写清 `risk_tags`、`mandatory_evidence`、`acceptance_cmds[*].kind` 的对应关系；
   - 命中 DB 变化时必须拆出 migration task；
   - 每条功能需求至少对应一条 UAT。
4. **高风险外部状态**：命中 `chat_db`、`data_db`、脚本链路或其它高风险外部状态时，计划里必须写出“风险 -> 命令 -> 证据”闭环；缺失按 `PLAN_DB_EVIDENCE_MISSING` 处理。
5. **DB 默认模板**：开发态使用 `bash scripts/db/run_dev_migration.sh`；若进入发布链路，再追加 Alembic。
6. **分流**：`next_step=/jjk-imp` 或 `next_step=/jjk-vkplan`。
