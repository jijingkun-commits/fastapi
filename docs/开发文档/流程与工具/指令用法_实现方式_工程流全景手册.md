...
4. 当前命令已分成两类：执行/编排类命令保留 `FAIL_FAST` 与硬门禁；分析/审查/重构/验收/测试类命令更偏“前置引导 + 明确产物 + 证据导向”。
...
1. **该门禁的门禁，该引导的引导**：执行/编排类命令优先 fail-fast；分析/审查类命令优先把结论、证据和产物说清。
...
| 测试 | `/jjk-test` | 风险驱动的可追溯测试矩阵执行与质量小结 | review/manifest/plan | `workdocs/任务拆解/<YYYY-MM-DD_主题>/reports/test_report.md` + 测试资产回填 |
...
| 辅助交接 | `/jjk-compact` | 把背景和任务压缩成可复制 prompt | 当前会话上下文 / 背景描述 / 文件路径 | 精简交接提示词 |
...
| 通用规划 | `/plan` | 轻量实施规划 | 设计文档 | `workdocs/设计/*-plan.md` |
...
1. **执行/编排类**：`输入前置 -> FAIL_FAST -> 执行流程 -> 证据回填 -> 下一步分流`
2. **分析/审查类**：`适用场景 -> 先读什么 -> 先报什么 -> 怎么判断 -> 产物骨架 -> 下一步分流`

后者目前主要覆盖：`/jjk-review`、`/jjk-debug`、`/jjk-refactor`、`/jjk-verify`、`/jjk-test`。
...
想看完整规范，回到 `.cursor/rules/command_authoring.mdc`。这里只记最容易反复踩坑的 5 条：

| 易回退点 | 正确写法 |
|---|---|
| 把全局规则塞进每个命令 | 全局写 `AGENTS.md` / Layer2；命令只写阶段特有信息 |
| 同时维护 command 和 skill 两套正文 | 只改 `.cursor/commands/*.md`；`SKILL.md` 只做镜像 |
| 为了“精简”删掉下游消费字段 | 保留 `risk_tags`、`mandatory_evidence`、`acceptance_cmds[*]`、`Required Evidence` 等锚点 |
| 只写原则，不给产物骨架 | 直接写字段结构、流程图、删除清单、示例 |
| 命中过往问题只改文案，不留回归锚点 | 先补最小合同测试，再改规则 |
...
2. **规划方式**：先解释为什么这样拆，再把任务、UAT、追溯矩阵和验证命令写到别人接手就能开工。
3. **关键要求**：每个 task 都能回溯到需求与方案，并写清 `risk_tags`、`mandatory_evidence`、`acceptance_cmds[*].kind` 的对应关系。
4. **高风险外部状态**：命中 DB、脚本链路或其它高风险外部状态时，计划里要让“风险 -> 命令 -> 证据”闭环一眼可见。
5. **分流**：`next_step=/jjk-imp` 或 `next_step=/jjk-vkplan`。

### `/jjk-imp`

1. **何时用**：已有可执行 `implementation_plan` 与 `uat_cases`。
2. **执行核心**：先对齐当前任务边界，再按计划实现、删旧、验证、回填，不反向改需求或方案。
3. **DB migration**：命中 `db_migration_required=true` 时，默认自动执行开发态迁移；若进入发布链路，再补发布态迁移证据。
4. **交接给验证**：回填 `acceptance_results`、`mandatory_evidence`、`db_migration_evidence`、文档同步状态和剩余风险。
...
| `workdocs/归档/正文/设计/2026-03-06-cardrun-wtimp-executor-design.md` | `cardrun` 迁移到 `wtimp` 默认执行器的设计取舍说明 |
| `memory-bank.md` | `cardrun` 默认执行器与上下文防错相关仓库级活跃决策索引 |
...
1. **审查方式**：finding-first，先写会影响放行的风险，再写简短结论。
2. **重点**：行为风险、设计漂移、结构退化、删除不完整、证据不足，以及 touched scope 的减法审查。
3. **产出**：`workdocs/任务拆解/<YYYY-MM-DD_主题>/reports/review_report.md`，附分级问题、证据与残余风险。

### `/jjk-test`

1. **测试方式**：先建风险模型，再建可追溯测试矩阵，再选最小必要测试。
2. **重点**：主失败模式覆盖、断言强度、低价值测试识别，以及“测试绿了是否真的能说明风险已收口”。
3. **产出**：`workdocs/任务拆解/<YYYY-MM-DD_主题>/reports/test_report.md`，包含测试矩阵、执行结果、测试质量小结和残余风险。

### `/jjk-verify`

1. **目标**：基于既有 `requirements/design/implementation_plan/uat_cases/review` 与最终证据给出 `PASS/WARN/FAIL`。
2. **验收方式**：先给结论倾向，再按追溯链、review 闭环、UAT、测试质量和运行态证据展开。
3. **执行约束**：上下文比对、解释器路径、测试语义分层与按需运行态校验继续遵循 `PLANS.md`，不在命令正文重复展开。
4. **结果**：输出 `workdocs/任务拆解/<YYYY-MM-DD_主题>/reports/verify_report.md`，明确写出为什么是这个结论。
...
1. **默认模式**：先报根因结论，再进入最小修复；`--diagnose-only`：只做诊断与修法对比，产出 `contracts/fix_plan.md`。
2. **引导顺序**：现象与影响 -> 根因证据 -> 是否继续修 -> 最小修复 -> 验证与文档回填。
3. **目标产出**：`reports/debug_report.md` 或 `contracts/fix_plan.md`，都要能看出“证据链 + 结论 + 下一步”。

### `/jjk-refactor`

1. **目标**：行为等价前提下改善结构，并把重构范围编译成 `imp` 可直接消费的执行合同。
2. **步骤**：先说清 baseline / obsolete_paths / retained_paths / single owner -> 再整理成精简 `implementation_plan` -> 需要时直连 `/jjk-imp` 或 `/jjk-imp-auto`。
3. **产物**：`contracts/implementation_plan.md`（可精简、可原位更新）+ `reports/refactor_report.md`。

## 5.6 通用补充命令

### `/plan`

1. 轻量实施规划命令，产出 `workdocs/设计/YYYY-MM-DD-<topic>-plan.md`。
