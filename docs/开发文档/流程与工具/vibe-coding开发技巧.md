...
3. 统计口径：按 `.cursor/commands/*.md` 文件计数（不含 `.bak`），统计时间 `2026-03-13`，当前共 `23` 个命令文件。
...
| 命令 | 说明 | 使用示例 |
|------|------|----------|
| `/jjk-compact` | 生成可复制到另一个 Codex 窗口的精简交接提示词 | `/jjk-compact 这里写背景和任务` |
| `/plan` | 通用实施规划，输出 `workdocs/设计/...` 计划文档（非 JJK 主链） | `/plan` |
...
# 交付与通用补充
/jjk-compact       # 生成另一个 Codex 窗口可直接使用的精简交接提示词
...
# === 命令执行节奏 ===
- 基础观测、期望比对、解释器解析、测试执行、统计汇总分步跑，不拼成长 `&&` / `;` 链
- 任一步失败只重跑当前步；长任务启动后只轮询/读结果，不重复启动
- 输出被截断时拆短当前步或分段读取，不为看尾部整串重跑
```
...
├── jjk-compact.md         # 生成另一个 Codex 窗口可直接使用的精简交接提示词
