...
| `DB_ECHO` | SQLAlchemy 原始 SQL 回显开关，仅建议临时排障时开启 | `false` |
...
| `ENABLE_DOCUMENT_MEMORY` | 用户个性化永久记忆总开关（纯文档） | `false` |
| `DOCUMENT_MEMORY_MAX_RESULTS` | 文档记忆召回片段上限 | `6` |
| `DOCUMENT_MEMORY_MAX_INJECTED_CHARS` | 文档记忆注入上下文字符预算 | `1200` |
| `DOCUMENT_MEMORY_VECTOR_WEIGHT` | 文档记忆混合检索向量权重 | `0.70` |
| `DOCUMENT_MEMORY_TEXT_WEIGHT` | 文档记忆混合检索关键词权重 | `0.30` |
| `DOCUMENT_MEMORY_HYBRID_MIN_SCORE` | 文档记忆混合召回最小综合分 | `0.05` |
| `DOCUMENT_MEMORY_EMBEDDING_BATCH_SIZE` | 每批补偿任务处理 chunk 数 | `32` |
| `DOCUMENT_MEMORY_EMBEDDING_MAX_RETRY` | 单 chunk 自动重试上限 | `3` |
...
| `SKILL_SIMILARITY_THRESHOLD` | 技能检索相似度阈值（环境变量兜底，主要用于 hybrid_rag） | `0.55` |
...
| `ENABLE_PROGRESSIVE_SKILL_LOADING` | 聊天主链是否启用 progressive loader | `true` |
| `SKILL_RUNTIME_MODE` | Skill 聊天运行模式：`catalog_tool/hybrid_rag` | `catalog_tool` |
| `ENABLE_SKILL_RUNTIME_TRACE` | 是否写 `additional_kwargs.skill_runtime` canonical 载荷 | `true` |
| `ENABLE_SKILL_CATALOG_METADATA_NORMALIZATION` | 是否优先读取 definition/version catalog 元数据 | `true` |
| `skill.runtime_source_mode` (DB) | Skill 运行时来源模式：`compat/strict_user` | `compat` |
| `skill.user_bootstrap_template` (DB) | 新用户 Skill 初始化模板（JSON） | `{"default_version":"v1","skills":[]}` |
...
> **文档化永久记忆配置说明（纯文档 + 单开关）**：
> - `ENABLE_DOCUMENT_MEMORY=true`：启用文档化永久记忆完整链路（写入/召回/管理接口）。
> - `ENABLE_DOCUMENT_MEMORY=false`：关闭文档化永久记忆链路（包括后台管理接口）。
> - `DOCUMENT_MEMORY_MAX_RESULTS` 与 `DOCUMENT_MEMORY_MAX_INJECTED_CHARS` 用于控制上下文注入预算，避免污染模型输入窗口。
> - `DOCUMENT_MEMORY_VECTOR_WEIGHT + DOCUMENT_MEMORY_TEXT_WEIGHT` 共同决定混合检索分数权重，建议总和为 `1.0`。
> - `DOCUMENT_MEMORY_HYBRID_MIN_SCORE` 控制混合召回最低阈值，阈值越高召回越保守。
> - `DOCUMENT_MEMORY_EMBEDDING_BATCH_SIZE`、`DOCUMENT_MEMORY_EMBEDDING_MAX_RETRY` 控制补偿吞吐与失败治理。

> **Skill Progressive Loader 说明（2026-03-07）**：
> - `ENABLE_PROGRESSIVE_SKILL_LOADING=true` + `SKILL_RUNTIME_MODE=catalog_tool`：默认走 catalog 预装 + `load_skills` 显式加载正文。
> - `SKILL_RUNTIME_MODE=hybrid_rag`：显式切回旧 hybrid 检索注入路径；该模式不再是默认主链。
> - `ENABLE_SKILL_RUNTIME_TRACE=true`：最终 AIMessage 统一写 `additional_kwargs.skill_runtime`，用于回放与恢复。
> - `ENABLE_SKILL_CATALOG_METADATA_NORMALIZATION=true`：catalog 文案优先读取 definition/version 的 `catalog_path/catalog_order/catalog_description/when_to_use`。
> - `ENABLE_SKILL_VERSIONING=true`：启用 `t_agent_skill_definitions` + `t_agent_skill_versions` 作为 runtime 真理源；`t_agent_skills` 仅保留兼容/导入/调试用途。
> - `ENABLE_USER_SKILL_BINDING=true`：启用 `t_user_skill_bindings` 用户级覆盖；关闭后忽略绑定并回退发布版本。
> - `skill.runtime_source_mode`：`compat` 保留 legacy 兼容回源，`strict_user` 禁止 legacy 主路径回退。
> - `skill.user_bootstrap_template`：统一维护新用户初始化模板（默认版本 + 技能清单），由导入流程自动兜底写入。
...
> **模型路由说明**：系统按能力需求将场景分为 5 层，通过后台管理「LLM 模型配置 -> 模型路由」页面配置：
...
>   - `vision`
>
> `t_llm_scene` 不再持久化 `default_model_id`，路由模型以 `t_system_config` 为唯一真源。
...
> **运维备忘（请定期执行）**  
> 建议每周至少执行一次 `python scripts/config_doctor.py --strict`，并在以下时点额外执行一次：  
> 1) 发布新版本前；2) 新增配置项后；3) 回滚/热修后。  
> 若发现 `t_system_config` 缺失配置键，请及时补齐，避免线上运行回退到默认值。
...
- 类型：`bool`
- 默认值：`false`
- 说明：SQLAlchemy `create_engine(..., echo=...)` 原始 SQL 回显开关。开启后会把 SQL 语句和绑定参数直接写入日志文件，适合短时排障，不适合作为常驻服务默认配置。

> 运维建议：
> - 默认保持 `DB_ECHO=false`，应用日志统一通过 `app/core/logging.py` 控制。
> - 若需要排查 ORM/SQL 细节，短时开启即可；排障结束后立即恢复为 `false`。
> - 当 `memory_intent_runtime`、定时任务、健康探针等后台链路常驻运行时，长期开启 `DB_ECHO` 会显著放大日志体积。
...
- 类型：`bool`
- 默认值：`false`
- 说明：用户个性化永久记忆单开关。开启后系统读写 `t_user_memory_document / t_user_memory_chunk`；关闭后不启用该链路。

### `USER_PREFERENCE_MEMORY_BOOTSTRAP_TEMPLATE` / `memory.user_preference_bootstrap_template`
- 类型：`json`
- 默认值：`{"assistant.persona":"小嘉"}`
- 说明：新用户创建成功后，会按该模板初始化 `doc_kind=preference` 的文档记忆项（当前支持 `response.language`、`response.length`、`response.structure`、`response.style`、`assistant.persona`）。

> 建议：生产环境至少保留 `assistant.persona` 模板项；如需停用初始化模板，可将配置置为 `{}`。

### `MEMORY_INTENT_ASYNC_ENABLED` / `memory.intent_async_enabled`
- 类型：`bool`
- 默认值：`false`
- 说明：聊天主链路记忆沉淀模式开关。开启后主链路仅向 `t_user_memory_intent_job` 入队，不再同步执行记忆判定；关闭后回退到历史同步写入链路。

> 运维建议：
> - 灰度发布时先保持 `memory.intent_async_enabled=false`，待 Worker 与观测链路就绪后再开启。
> - 当 `feature.enable_document_memory=true` 且 `memory_intent_runtime` 已接通时，开发/日常交互环境建议保持 `memory.intent_async_enabled=true`，避免聊天首包被同步记忆判定阻塞。
> - `memory_intent_runtime` 空闲时采用退避轮询；若需要观察原始 SQL，请只在短时排障窗口临时开启 `DB_ECHO=true`。
> - 该开关变更后，已启动进程至少要刷新 `SystemConfigService` 缓存；若此前 Worker 处于 `memory_intent_runtime_disabled`，还需要重启后端，让 `lifespan -> start_memory_intent_runtime()` 重新判定并拉起消费者。
> - 回滚路径统一使用 `memory.intent_async_enabled=false`。
