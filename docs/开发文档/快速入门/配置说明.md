...
| `ENABLE_USER_PREFERENCE_MEMORY` | 跨会话用户偏好记忆总开关 | `false` |
| `ENABLE_MEMORY_RECALL` | 偏好记忆召回开关（可独立于总开关灰度） | 跟随 `ENABLE_USER_PREFERENCE_MEMORY` |
| `ENABLE_PRE_COMPACTION_FLUSH` | 偏好记忆写入开关（压缩前 flush） | 跟随 `ENABLE_USER_PREFERENCE_MEMORY` |
...
> **模型路由说明**：系统按能力需求将场景分为 4 层，通过后台管理「LLM 模型配置 -> 模型路由」页面配置（持久化到 `t_system_config` 表）：
> - `model_routing.sql_generation`：SQL 生成 / 内部分析（标准模型，推荐 qwen-plus）
> - `model_routing.lightweight`：轻量任务（意图分类/参数提取/评估，推荐 glm-4.5-air）
> - `vision`：Vision 多模态路由（支持绑定 `vision/chat/reasoning` 类型模型，未配置回退 `type=vision` 默认模型）
> 
> 环境变量 `LLM_JUDGE_MODEL` / `INTENT_CLASSIFIER_MODEL` 仅作为数据库未配置时的回退默认值。
...
- 类型：`bool`
- 默认值：`false`
- 说明：跨会话用户偏好记忆总开关。关闭后系统不会读取或写入 `t_user_memory`。
...
- 类型：`bool`
- 默认值：继承 `ENABLE_USER_PREFERENCE_MEMORY`
- 说明：偏好记忆“读路径”开关。关闭后不注入历史偏好上下文，但不影响主对话流程。

### `ENABLE_PRE_COMPACTION_FLUSH`
- 类型：`bool`
- 默认值：继承 `ENABLE_USER_PREFERENCE_MEMORY`
- 说明：偏好记忆“写路径”开关。开启后会在保存 human 消息后尝试提取并写入偏好。

> 建议：先在灰度环境开启 `ENABLE_USER_PREFERENCE_MEMORY=true`，观察日志与响应质量后再全量启用。
