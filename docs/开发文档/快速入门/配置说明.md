...
| `ENABLE_DOCUMENT_MEMORY` | 文档化永久记忆总开关（两表方案） | `false` |
| `ENABLE_DOCUMENT_MEMORY_RECALL` | 文档记忆召回开关（`memory_search + memory_get`） | `false` |
| `ENABLE_DOCUMENT_MEMORY_FLUSH` | 文档记忆写入开关（显式记忆 + 预压缩前沉淀） | `false` |
| `ENABLE_DOCUMENT_MEMORY_HYBRID_SEARCH` | 文档记忆混合检索开关（FTS + 向量） | `false` |
| `ENABLE_DOCUMENT_MEMORY_EMBEDDING_WORKER` | 文档记忆向量异步补偿开关 | `false` |
| `ENABLE_DOCUMENT_MEMORY_ADMIN_API` | 文档记忆后台运维 API 开关 | `false` |
...
> **文档化永久记忆配置说明（两表方案）**：
> - `ENABLE_DOCUMENT_MEMORY=true`：启用文档化记忆总开关，主存储为 `t_user_memory_document + t_user_memory_chunk`。
> - `ENABLE_DOCUMENT_MEMORY_RECALL=true`：启用文档记忆召回链路；关闭时回退到 KV 控制层记忆。
> - `ENABLE_DOCUMENT_MEMORY_FLUSH=true`：启用文档记忆写入链路；关闭时不写入文档层。
> - `ENABLE_DOCUMENT_MEMORY_HYBRID_SEARCH=true`：启用 FTS + 向量混合检索；关闭时自动回退纯 FTS。
> - `ENABLE_DOCUMENT_MEMORY_EMBEDDING_WORKER=true`：启用异步向量补偿 worker；关闭时仅保留文本检索链路。
> - `ENABLE_DOCUMENT_MEMORY_ADMIN_API=true`：开放后台重建/重试接口，仅管理员可访问。
...
> - `skill.runtime_source_mode`：`compat` 保留 legacy 回退，`strict_user` 禁止 legacy 主路径回退。
