...
| `SKILL_SIMILARITY_THRESHOLD` | 技能检索相似度阈值 | `0.55` |

> **技能检索阈值说明**：该值控制 Skills RAG 检索的过滤门槛。值越高，匹配越精准但可能漏召回；值越低，召回率高但可能匹配到不相关技能。推荐范围 0.5-0.6。也可通过数据库 `t_system_config` 表的 `skill_similarity_threshold` 配置项动态调整。
...
> **模型路由说明**：系统按能力需求将场景分为 3 层，通过后台管理「LLM 模型配置 -> 模型路由」页面配置（持久化到 `t_system_config` 表）：
...
| `TEST_BACKEND_PORT` | `/test` 在线检查后端端口 | 跟随 `VK_BACKEND_PORT` / 主分支 `8000` |
| `TEST_FRONTEND_PORT` | `/test` 在线检查前端端口 | 跟随 `VK_FRONTEND_PORT` / 主分支 `3000` |
...
| 配置键 | 说明 | 默认值 |
|--------|------|--------|
| `askdata.schema_whitelist` | 允许查询的 Schema 白名单 | `fdmdata,sdmdata,public` |
