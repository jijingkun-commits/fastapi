...
18. hard cancel 成功后，对应 run 必须立刻从 active run 视图中移除，并释放 1 个并发槽位。
19. 历史侧栏只允许 `running` 显示旋转运行图标；`stopping` 不得继续伪装为运行中。
20. 历史侧栏的未读蓝点只表示“存在未读回复且当前不在运行”；已读后必须清除，不得与运行态图标叠加。
