...
7. 2026-03-10 起，聊天主链路涉及的预构建 Agent 入口统一使用 `langchain.agents.create_agent`；`create_react_agent` 不再作为生产路径 API。
8. 本轮迁移只覆盖 Supervisor / knowledge agent 等预构建入口，不把 `TodoGraph`、`DataGraph`、多智能体主图整体改写成 Functional API。
9. 迁移后必须继续满足 `interrupt / resume / replay` 语义不变，以及 `messages / values / custom` 三路流式契约不漂移。
