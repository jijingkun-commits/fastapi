...
11. `stream` 与 `resume` 必须复用同一套生命周期收口规则：运行态真相源仍为 `GET /api/v1/chat/runs/active`，SSE 负责补充 `token/result/status/interrupt/done` 事件，禁止前端再派生第二套脱离真相源的运行态状态机。
...
- `token`: `{content}`
- `status`: `{message, phase?}`（`phase` 缺省时按 `processing` 处理）
- `result`: `{data_type, data, message?}`
- `interrupt`: `{thread_id, interrupt_id, value}`
- `done`: `{thread_id, message_id?, final_content?, status?}`（仅用于生命周期收口；若正文尚未真正发出，可用 `final_content` 做一次补发；`status` 可用于未读/运行态清理判定）
- `error`: `{message}`
...
6. 若本轮先出现 `result` 占位卡片、但未产生正文 token，stream/resume 收口时仍必须补发最终 AI 正文，不得只留下图片/卡片没有文字总结。
