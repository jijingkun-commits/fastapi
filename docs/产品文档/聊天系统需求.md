...
13. 第三方搜索工具原始网页/HTML 噪声不得直接进入 handoff frame、coverage 汇总或最终答复。
14. `data_expert` 仅返回失败/澄清文本、未产出结构化结果时，不得判定为 `success`；若存在失败摘要则标记为 `failed` 并向最终答复透出失败原因，若无结构化结果也无失败摘要则标记为 `missing`。
15. `external.lookup` 的用户可见正文只认 `payload.display_markdown`；若 Supervisor 已先给出可直答 Markdown，最终展示必须优先保留该段 Markdown，不得再从原始 tool 文本反推正文。
