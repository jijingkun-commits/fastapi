...
7. 对编号列表、分行等显式多问题输入，服务端应允许在 `preprocess` 直接生成 goals，并尽早回流 `plan_ready` 或已完成子结果，避免所有子问题都等待 Supervisor 完整往返。
8. 复合问题中的公开结构化事实查询按 `public_structured_fact` 通用能力处理，与内部数据查询分层；不得在编排层通过天气等单题关键词继续堆叠特殊路由。

7. 2026-03-10 起，聊天主链路中可独立迁移的预构建 Agent 入口统一使用 `langchain.agents.create_agent`；当前多智能体 Supervisor 因运行时工具裁剪仍保留 `create_react_agent`。
8. 本轮迁移不把 `TodoGraph`、`DataGraph`、多智能体主图整体改写成 Functional API，也不在 runtime gating 尚未等价前强改 Supervisor 构建方式。
...
6. 显式双问题场景下，已完成的公开事实子结果应可先于慢子任务提前回流，最终答复再统一收口。
...
5. `sql_result` 渲染必须保持统一视觉契约：表格与 SQL 折叠区共用聊天区设计 token，不允许结果卡片再维护独立代码块主题。
...
| SQL 结果卡片视觉契约 | 表格与 SQL 折叠区复用聊天区共享 Table / CodeBlock 规范 | `sql_result_visual_contract` |
...
5. `sql_result` 场景下，长 SQL 展开后表格仍保持整卡宽度展示，代码块风格与普通 Markdown 代码块一致。
