...
14. Word 附件当前 P0 仅承诺支持 `.docx`；用户选择 `.docx` 后，输入区预览、发送成功后的消息区、刷新后的历史回放必须都能看到同一份文件附件，不得一处可见、一处消失。
15. 老 `.doc` 不在当前支持范围内；用户选择或上传 `.doc` 时，前后端必须统一提示 `暂不支持 .doc，请先转换为 .docx`，不得出现上传接口放行、前端却提示“不支持的文件类型”的双口径。
16. `.docx`、`.pdf`、`.xlsx` 等非图片附件，在 human 历史消息中都必须以可点击文件链接稳定回放；不得因为文件类型不同而出现“这类文件能刷新回放、那类文件不能”的行为差异。
17. 对已在支持范围内的 `.docx`，若文件损坏、内容异常或解析失败，系统必须给出“文件可上传但读取失败”的人话反馈，不得误导用户为“格式不支持”。
18. 复合请求中，天气、知识库、图表、待办、问数等独立诉求必须按原子 goal 执行并按用户表达顺序进入最终答复；不得把多个诉求合并成单个 bucket 结果后宣称“已全部覆盖”。
19. `todo.query` 默认不得被天气、知识库等外部 observation 污染；只有当前子任务明确要求“结合外部结果回复/汇总”时，才允许附带结构化 observation。
20. 图表类结果必须同时进入最终正文与结构化结果；天气富文本不得吞掉同轮知识库、待办或问数结果。
21. `supervisor` 必须继续作为唯一主会话 owner；一期只允许 1 个统一 `research_subagent`，首批范围只覆盖 `knowledge + web` 多来源研究、对比、归纳与证据汇总。
22. research 升级条件必须由 `intent/goal_resolver` 的 goal bucket 决定；单次 `knowledge_search`、单次 `search_tool` 仍走 direct tool 快路径，不得因为底层能力来自知识库或联网就默认升级为 `research_subagent`。
23. `research_subagent` 对主会话返回的结果必须是结构化合同，至少包含 `summary / evidence / insufficiency`，并允许携带 `media_refs`；研究失败或证据不足时，必须返回结构化 insufficiency，不得把原始网页、HTML 噪声或研究 scratchpad 直接透传给用户。
24. 附件必须保持 route-agnostic：所有附件先进入 supervisor planning，再结合用户真实目标选择 `direct_tool / data_workflow / todo_workflow / research_subagent / mixed`；不得因为“有附件 / 多附件 / document probe”就默认切 research。
25. research 场景下的知识库图文展示必须继续复用 canonical `display_blocks / kb_images` 链路，保证 live SSE 与 history replay 同构；即使图片失败，也必须保留文本结论与证据降级，不能退化为只剩原始占位符或空结果。
...
6. 送模前上下文必须由单一构造入口统一装配；工作流编排层不得继续散落拼接系统提示、技能上下文与历史消息。
7. Skill 送模默认应优先使用 `loaded_skill_registry` 的摘要与结构化元数据，不得默认把 `loaded_skill_context` 全量正文直接回灌给模型。
8. 系统需对 `prompt`、`tool schema`、`system`、`skills`、`messages` 五类送模成本做 token 级可观测统计，用于预算治理、压缩回归与性能对比。
...
6. 问数等业务子图若需要追问，聊天主链路只能消费结构化 contract（如 `router_result_v2.route_decisions[].data_intent.clarify`）生成用户可见澄清；不得在编排层再从文案倒推缺失槽位，也不得让影子判定链路改写主回复。
7. 工具调用与工具结果若对用户可见，其卡片必须跟聊天主界面共享统一视觉语言，并收口为内容宽度优先的紧凑组件；不得额外露出独立的系统风格条块。
8. 聊天页前端壳层、状态条、输入区与消息工具条必须收口到单一视觉 owner；不得并存第二套平行 inline 壳层样式。
9. 运行态状态条必须展示在消息流尾部，并与消息内容列宽对齐；footer 只承载输入区，不得再次承载运行态状态提示。
10. 前端图片渲染默认不得继续裸用 `<img>` 作为通用实现；静态品牌图优先 `next/image`，动态内容图若需保留原始地址或运行时尺寸，必须显式说明并采用受控方案。
...
2. 结构化结果输入继续统一通过 `additionalKwargs.result_events[]` 传递，并保持 `read-old-write-new` 兼容。
3. AI 最终展示统一收敛到 `message.content` 的 **ordered content blocks**；当 `content_type=multimodal` 时，必须按最终展示顺序返回 `markdown / image / sql_result / todo_list / fallback_result`。
4. live 流式阶段必须消费 SSE `display_blocks` 快照；前端不得再根据 `contentString + kb_images + result_events` 现场编译展示块。
5. 知识库图片允许继续使用 `[IMG-N]` 作为中间锚点，但渲染层不得再把占位符替换逻辑当作长期 owner。
6. 单轮多个结构化结果必须按追加顺序保留，不得以后到结果覆盖先到结果。
7. 反馈接口只接受数据库整数 `message_id`。

### 4.3 结构化结果与重连契约

1. 复合提问（文本 + 表格 + 图片）场景下，结构化响应统一走 `result` 事件，不允许并行私有协议。
2. `result_events[]` 是 canonical **输入契约**；`message.content(blocks)` 是 canonical **最终展示契约**。两者必须同轮同构，禁止双轨漂移。
3. 历史回放优先消费 `content_type=multimodal` + `content=[blocks...]`；旧消息若仍是 markdown + `kb_images/result_events`，必须在后端接口层编译成 blocks 后再返回。
4. SSE 可靠性最小集固定为 `id / retry / heartbeat / Last-Event-ID`，用于断线重连、续传与去重恢复。
...
17. 问数类纯指标问题若缺关键执行槽位，最终用户看到的澄清必须与运行态结构化 contract 同口径；影子对账即便发现差异，也只能记录观测，不得改变主路径答复。
18. 复合请求中，普通 `todo.query` 不得因外部 observation 误判为 `out_of_scope`；但“查询待办并结合天气结果回复用户”这类明确组合场景仍应保留必要 observation 并正常完成。
...
| CHAT-TC-002       | 当前轮消息切片正确性（含历史消息场景）  | `tests/unit/test_chat_service_turn_slice.py`、`tests/unit/test_chat_service_human_attachment_persistence.py` |
...
| CHAT-TC-010       | mixed query 下精确 data handoff 保留    | `tests/unit/test_handoff_detection.py`                                          |
| CHAT-TC-011       | Tavily observation 归一，网页噪声不透传 | `tests/unit/test_multi_agent_streaming_helpers.py`                              |
| CHAT-TC-012       | 运行态状态条挂在消息流尾部且宽度对齐    | `tests/unit/test_chat_runtime_status_layout_guard.py`、`web/e2e/chat-runtime-status.spec.cjs` |
| CHAT-TC-013       | 新会话按钮与历史点击切换保持稳定        | `web/e2e/chat-thread-switch.spec.cjs`                                           |
...
| CHAT-CQ-TC-003    | 当前轮切片避免跨轮污染；历史消息存在时仍只补发当前轮 AI 正文 | `tests/unit/test_chat_service_turn_slice.py`、`tests/unit/test_chat_service_human_attachment_persistence.py` |
