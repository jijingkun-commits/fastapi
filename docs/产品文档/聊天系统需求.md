...
5. 工具调用与工具结果若对用户可见，其卡片必须跟聊天主界面共享统一视觉语言，并收口为内容宽度优先的紧凑组件；不得额外露出独立的系统风格条块。
6. 运行态目标合同在单目标 direct handoff 与 `decompose_goals` 显式拆解两条路径上必须一致，禁止出现双真理源状态。
7. 聊天页前端壳层、状态条、输入区与消息工具条必须收口到单一视觉 owner；不得并存第二套平行 inline 壳层样式。
8. 运行态状态条必须展示在消息流尾部，并与消息内容列宽对齐；footer 只承载输入区，不得再次承载运行态状态提示。
9. 前端图片渲染默认不得继续裸用 `<img>` 作为通用实现；静态品牌图优先 `next/image`，动态内容图若需保留原始地址或运行时尺寸，必须显式说明并采用受控方案。
...
17. 复合请求中，普通 `todo.query` 不得因外部 observation 误判为 `out_of_scope`；但“查询待办并结合天气结果回复用户”这类明确组合场景仍应保留必要 observation 并正常完成。
