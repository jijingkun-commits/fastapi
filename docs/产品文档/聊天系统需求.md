...
14. 复合请求中，天气、知识库、图表、待办、问数等独立诉求必须按原子 goal 执行并按用户表达顺序进入最终答复；不得把多个诉求合并成单个 bucket 结果后宣称“已全部覆盖”。
15. `todo.query` 默认不得被天气、知识库等外部 observation 污染；只有当前子任务明确要求“结合外部结果回复/汇总”时，才允许附带结构化 observation。
16. 图表类结果必须同时进入最终正文与结构化结果；天气富文本不得吞掉同轮知识库、待办或问数结果。
