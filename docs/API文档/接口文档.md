...
1. 仅 run 所属用户或管理员可取消。
2. 目标 run 已处于 `stopping/stopped/completed/failed` 时，返回 `accepted=true`、`idempotent=true`。
3. 当前实现固定 `cancel_mode=hard`；取消成功后 run 直接返回 `stopped`，并从后续 active 列表中移除。
4. 前端侧边栏仅对 `status=running` 显示 spinner；`stopping` 不得继续显示为运行中。
