...
获取总览摘要快照。V2 口径下，页面按“系统状态、业务请求质量、用户提问活跃度、稳定性、容量与成本、数据新鲜度、模块健康矩阵、实时告警/关键变更流”八块信息展示。

**数据口径说明**:

1. 默认统计窗口为最近 5 分钟，`summary`、`trends`、`stream` 统一读取同一套分钟聚合读模型，禁止分别依赖实时临时计算与历史展示快照。
2. `request_quality` 表示 **全业务 API 请求质量**，统计范围为业务接口请求；`question_activity` 表示 **用户提问活跃度**，仅统计聊天提问入口，请勿混用两者语义。
3. 页面状态统一显式输出 `ok / no_data / stale / degraded`：`ok` 表示当前窗口有可用样本，`no_data` 表示窗口内无样本，`stale` 表示有旧样本但已过期，`degraded` 表示聚合链路异常或查询结果被动降级；该状态不得再通过旧展示快照表伪装成正常数据。
4. `/api/v1/admin-overview/*` 自身请求不得反向污染业务请求质量或提问活跃度统计。
5. 健康总分仅在存在足够业务样本且关键维度可计算时输出；“系统在线”与“业务无样本”必须拆开表达，禁止在无样本场景下输出误导性正常分值。
...
```json
{
  ...
  "system_status": {
    ...
    "watermark_at": "2026-03-09T01:38:48Z",
...
2. 趋势维度至少覆盖健康总分、全业务 QPS、用户提问 QPS、预算占用等核心指标。
...
```
event: result
data: {"type":"result","data":{"snapshot_at":"2026-03-09T01:38:50Z","patch":{"traffic_health":{"status":"no_data","sample_count":0},"request_quality":{"status":"no_data"}},"trace_id":"trace-stream-v2-001"}}
