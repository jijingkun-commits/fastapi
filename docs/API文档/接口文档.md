...
3. 取消成功后，流式通道发 `stopped` 事件并停止后续 token。
...
更新模型路由配置（支持 `fixed_config` 及 `vision`）。

> 当 `config_key=vision` 时，接口会切换 `t_llm_model` 中 `model_type=vision` 的默认模型（`is_default`），并刷新 LLM 配置缓存。
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `config_key` | string | 是 | 配置键名（`model_routing.default_chat` / `model_routing.lightweight` / `model_routing.sql_generation` / `vision`） |
| `model_code` | string | 是 | 目标模型代码（必须是已启用模型；`config_key=model_routing.default_chat` 时必须为 chat，`config_key=vision` 时必须为 vision） |
...
| Code | Description |
|------|-------------|
| 400 | 不支持的配置项、模型不存在/未启用、或模型类型与路由不匹配 |
...
| `default_model_id` | int | 默认模型 ID |
| `default_model_code` | string | 默认模型代码 |
...
1. 默认采集窗口为最近 5 分钟 API 运行时观测（请求量、5xx、P95、QPS），且会排除 `/api/v1/admin-overview/*` 自身请求，避免页面刷新/轮询导致总请求量被放大。
