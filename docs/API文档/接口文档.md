...
> 配置键口径：服务端主写 `askdata.*`，并兼容读取历史键（`data_access.*`，以及 `askdata.schema_whitelist`）。
...
```json
{
  ...
  "schema_whitelist": ["pg_catalog", "information_schema"]
}
```
...
获取 Schema 白名单（历史字段名，实际用于系统 Schema 禁止列表配置）。

#### PUT /access-admin/schema-whitelist

更新 Schema 白名单（历史字段名，实际写入 askdata.schema_blacklist）。
...
1. 默认采集窗口为最近 5 分钟 API 运行时观测（请求量、5xx、P95、QPS）。
2. 当窗口内无请求流量时，返回 `overview.runtime.no_traffic` 提示告警，健康分字段可能为 `null`。
