...
data: {"blocks": [{"type": "markdown", "data": {"text": "按你的问题顺序："}}, {"type": "image", "data": {"url": "/api/v1/assets/proxy/ragflow/img-0", "source": "knowledge"}}]}
...
5. **最终展示单一快照**：live 渲染必须消费 `display_blocks.blocks` 作为 canonical 展示块快照，前端不得再用 `contentString + kb_images + result_events` 现场拼 UI。
6. **`final_content` 仅作补发兜底**：若本轮先产生 `result` 占位、但尚未真正输出正文 token，流结束或 resume 收口时仍必须允许通过 `done.final_content` 补发最终 AI 文本，避免正文丢失。
7. **显式复合问题允许快速拆解**：对于编号列表/分行等显式多问题输入，服务端可直接生成 goals，必要时从 `preprocess` 直接路由到对应专家，跳过额外 planner / supervisor 往返；`decomposed_goals` 仍是唯一目标状态源。
8. **已完成子问题允许提前回流**：复合任务执行中，已完成的直答子问题可先通过 `token` 增量返回；`final_answer` 继续承担最终合并结论。
...
| `display_blocks` | final_answer/result/kb_images 收敛后的 canonical 展示块快照 | 聊天主消息最终布局 |
...
[
  {
    ...
    "content": [
      {"type": "markdown", "data": {"text": "**贷款余额** 1,311.07 亿"}},
      {"type": "image", "data": {"url": "/api/v1/assets/proxy/ragflow/img-0", "source": "knowledge"}},
      {"type": "sql_result", "data": {"sql": "...", "columns": ["机构"], "rows": [{"机构": "总行"}]}}
    ],
    "metadata": {"result_events": [{"data_type": "sql_result", "data": {"sql": "..."}}]},
    "additional_kwargs": {"result_events": [{"data_type": "sql_result", "data": {"sql": "..."}}]},
    "title": null,
...
| 字段 | 类型 | 说明 |
|------|------|------|
| `content_type` | string | `text / markdown / mixed / multimodal`；其中 AI 图文混排主路径使用 `multimodal` |
| `content` | string / array | 当 `content_type=multimodal` 时，返回有序内容块数组，按最终展示顺序组织 `markdown / image / sql_result / todo_list / fallback_result` |
| `feedback_score` | int / null | 当前用户的反馈: 1=赞, -1=踩, null=无反馈 |
| `additional_kwargs` | object / null | 结构化结果原始回放载荷；`result_events[]` 仍是 canonical 输入，但不再直接决定最终布局 |

> 兼容说明（2026-03-12）：
> 1. 当历史消息已按 `multimodal` 落库时，接口直接返回块数组，不再打平成纯文本。
> 2. 当历史消息仍是 `markdown` 但携带 `kb_images` / `result_events[]` 时，接口会在后端编译成 `multimodal blocks` 后再返回，前端不再做 placeholder 编译。
> 3. 当历史记录中存在遗留结构串（例如 `[{"type": "text", "text": "..."}]` 或 Python repr 形式）时，接口会继续归一化为可读正文后返回 `content`。
...
| 文档 | application/pdf, application/vnd.openxmlformats-officedocument.wordprocessingml.document, text/plain, application/json |
| 表格 | application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel, text/csv, application/csv |

说明：
- Word 文档当前仅支持 `.docx`
- 老 `.doc` 不在接口承诺范围内，建议用户先转换为 `.docx`
...
> 说明（2026-02）：该接口仅通过 `apply_lookup_enrichment_rule_with_status()` 执行规则测试，
