...
- canonical 字段：`message.additional_kwargs.result_event`。
- 写路径（新）：只写 `additional_kwargs.result_event`。
- 读路径（兼容）：
  1) `additional_kwargs.result_event`（新）
  2) `additional_kwargs.data_type + additional_kwargs.data`（旧）
  3) `metadata` 历史兼容字段（仅读）
...
requirement_seeds:
  - design_item: D-01
    ...
    input_contract:
      ...
      optional_fields: [message, event_id, sequence_number, timestamp, event_version]
      defaults:
        message: ""
    output_contract:
  ...
  - design_item: D-03
    ...
    input_contract:
      ...
      optional_fields: [additional_kwargs.result_event, additional_kwargs.data_type, additional_kwargs.data, metadata]
      defaults: {}
    output_contract:
...
implementation_seeds:
  ...
  - task_id: T-03
    ...
    symbols:
      - additional_kwargs.result_event
      - read_old_write_new
      - compat_source
    change_type: modify
    blocked_by: [T-01]
...
clarify_handoff_contract:
  ...
  extended:
    observability_hints:
      - 统一日志键 trace_id/thread_id/run_id/data_type/reason_code/event_version
...
- approval_evidence: "待用户确认"

> 以上设计已完全冻结。  
> 请回复：**确认 / 需要修改XX点 / 否**  
> （回复“确认”或“是”且门禁全部通过即视为审批通过，可进入 `$jjk-plan`）
...
execution_notes:
  ...
  degrade_reason: ""
  alternative_tool: ""
  verification: "已完成仓库上下文扫描 + 联网与 GitHub 证据检索"
```
