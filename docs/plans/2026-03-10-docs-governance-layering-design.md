...
> 更新时间：2026-03-10
> 设计状态：`approved`
> 关联方案：`docs/plans/2026-03-08-doc-single-source-dynamic-governance-design.md`
...
- `docs/plans/`、`docs/内部参考/迭代需求/`、`docs/内部参考/任务拆解/` 当前仍保留为迁移期旧过程路径；`task_split` 下的 `_active_task.json`、`vk_cards.json`、`preflight_status.json`、`consumption_report.json` 等机器契约/过程报告 JSON 暂不强迁。
...
- 成功标准:
  ...
  - 新增主题不再同时出现在 `docs/plans/`、`docs/内部参考/迭代需求/`、稳定文档三个位置表达“当前态”。
...
| 过程文档层 | 沉淀需求、方案、任务拆解、评审、验收证据 | `workdocs/需求`、`workdocs/方案`、`workdocs/任务拆解`、`workdocs/评审`、`workdocs/验证` |
...
2. 设计冻结进入 `workdocs/方案/`，实施与拆解进入 `workdocs/任务拆解/`。
...
2. `docs/plans/` 与 `docs/内部参考/迭代需求/` 不再双轨并存，统一归到 `workdocs/方案/` 与 `workdocs/需求/`。
3. `docs/内部参考/任务拆解/` 不再作为文档主区的一部分，迁入 `workdocs/任务拆解/`。
...
| T02 | [T01] | `docs/内部参考/**`,`workdocs/需求/**`,`workdocs/方案/**`,`workdocs/任务拆解/**` | `process_layer_rehome`,`internal_reference_reduction` | refactor |
| T03 | [T02] | `.artifacts/runs/**`,`.artifacts/states/**`,`.artifacts/generated/**`,`docs/内部参考/任务拆解/**` | `runtime_artifact_rehome`,`state_file_cleanup` | refactor |
...
| T05 | [T02,T03,T04] | `docs/开发文档/工作流/文档治理基线清单.md`,`docs/开发文档/工作流/文档月度校准清单.md` | `governance_checklist_updates` | modify |
...
clarify_handoff_contract:
  version: v2
  topic: 文档分层治理与信息架构收敛
  design_source: docs/plans/2026-03-10-docs-governance-layering-design.md
  handoff_ready: true
  required:
