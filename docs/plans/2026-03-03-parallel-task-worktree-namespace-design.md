...
- 范围: `scripts/wt-flow.sh`、`scripts/coder4_bootstrap_kernel.py`、`scripts/coder4_vk_sync.py`、`scripts/check_integration_gate.py`、相关流程文档（`jjk-cardrun`/`jjk-vkplan`/`jjk-vktodo`）。
...
| B. 仅改 `wt-flow.sh` 命名空间 | 解决分支/worktree/会话冲突，主流程改动集中 | 读侧脚本仍读全局 `.omc/state`，会出现“写隔离、读错位” | 中 | 中 |
...
- 架构:
  ...
  - 统一资源命名为 task_key 作用域:
    ...
    - 会话状态: `.omc/state/<task_key>/wt-flow-state.json`
    - 卡片状态: `.omc/state/<task_key>/task-runner-state.json`
    - 证据目录: `.omc/state/<task_key>/attempts/<card_id>/`
...
- 组件:
  ...
  - `scripts/coder4_bootstrap_kernel.py`
    - 默认状态路径从固定 `.omc/state/task-runner-state.json` 升级为“按 active_task 推导的 task_key 路径”。
...
- 异常与测试考虑:
  ...
  - 回归用例:
    - 同时准备任务 A/B（均含 `C01`），验证分支/worktree/state/attempts 全部隔离。
    - 在 A、B 间切换 active 多次，验证 `next/verify/merge/list` 始终命中当前任务状态。
    - 兼容测试: legacy `feature/C01` + `.worktrees/C01` 场景仍可正常收口。
  - 文档同步:
    - 同步更新 `.cursor/commands/jjk-cardrun.md`、`.cursor/commands/jjk-vkplan.md`、`docs/开发文档/工作流/开发工作流.md` 中旧路径示例。

## 5. 未决问题（如有）
- [ ] 是否在 `_active_task.json` 显式新增 `state_root` 字段，减少多脚本重复推导。
- [ ] 是否提供一次性迁移脚本，将 legacy `.omc/state/task-runner-state.json` 迁移到 `.omc/state/<task_key>/`。
- [ ] `.cursor/scripts/wt-flow.sh` 与 `scripts/wt-flow.sh` 的维护策略是否统一为单一真理源（避免双份漂移）。
