...
- `workdocs/任务拆解/**` 与 `.artifacts/states/task_splits/**` 属于**任务产物层**（计划态、作用域态、运行态分层沉淀）。
...
| 任务 JSON 文件数 | 33 | `workdocs/任务拆解/**/*.json` + `.artifacts/states/task_splits/**/*.json` |
...
- `workdocs/归档/专题/治理专题/工程减法治理/工程减法体检报告_2026-03-06_v2.md`

### 9.2 下一步建议

1. 以 `v2` 为准回写原报告，避免团队继续执行高风险删除命令。  
2. 立项一个小卡：`门禁脚本统一入口 + wrapper 兼容退役`。  
3. 在 `workdocs/README.md` 增加“过程产物生命周期”规范段落。  
