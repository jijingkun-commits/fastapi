...
- 边界:
  ...
  - 不引入新的任务系统，仅在现有 `.artifacts/states/task_splits/<task_split_dir>/<task_key>` 与 `scripts/*` 体系上收敛。
- 成功标准:
...
- 架构:
  ...
  - 证据数据面：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::merge_results.<card_id>`、`task-runner-state.json`。
  - 审计与展示面：`workdocs/任务拆解/*` 与 CI Summary。
- 组件:
...
- 数据流:
  ...
  - Step 2（证据检查）：读取 `.artifacts/states/task_splits/<task_split_dir>/<task_key>`，确认 C01~C07 的 `merge_result.json` 完整且格式正确。
