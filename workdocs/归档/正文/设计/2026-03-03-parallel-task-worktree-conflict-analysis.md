...
- 会话状态文件：`STATE_FILE=.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json`（第 21 行，记录当前 worktree 会话）
...
- Active 任务索引：`ACTIVE_TASK_FILE=workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json`（第 22 行）

**关键约束**（参考 `workdocs/README.md:64`）：
...
| **类型3：状态冲突**（分两个子类） | | | |
| 　└ **3a：会话状态冲突** | `STATE_FILE=.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json` | 当前 worktree 会话被覆盖 | `wt-flow.sh:21`（STATE_FILE 定义）<br>`wt-flow.sh:406-417`（_save_state） |
...
| **类型4：Active 索引冲突** | `workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json` | 单一 active 索引被多个任务争抢 | `wt-flow.sh:22`（ACTIVE_TASK_FILE）<br>`README.md:64`（单 active 约束）<br>`set_active_task.py:132`（写入逻辑） |
...
`wt-flow.sh` 使用单一会话状态文件 `STATE_FILE=.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json` 记录当前 worktree 会话（branch/worktree/base_branch），多个任务并行时会相互覆盖。

**根因分析**（`scripts/coder4/wt-flow.sh:21, 406-417`）：

```bash
# 第 21 行：STATE_FILE 定义
STATE_FILE="${REPO_ROOT}/.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json"
...
```bash
# wt-flow.sh:22
ACTIVE_TASK_FILE="${REPO_ROOT}/workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json"
...
任务A (task_key=planner-refactor)
  ...
  └─ 独立分支命名空间：feature/planner-refactor/*
     ...
     独立会话状态：.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-planner-refactor.json
     独立卡片状态：.artifacts/states/task_splits/<task_split_dir>/<task_key>/planner-refactor/task-runner-state.json

任务B (task_key=todo-enhance) ← 并行开发
  ├─ C01 (串行) → C02 (串行)
  └─ 独立分支命名空间：feature/todo-enhance/*
     独立 worktree 目录：${WT_BASE}/todo-enhance/*
     独立会话状态：.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-todo-enhance.json
     独立卡片状态：.artifacts/states/task_splits/<task_split_dir>/<task_key>/todo-enhance/task-runner-state.json
```
...
_session_state_file() {
  ...
  if [[ -n "$task_key" ]]; then
    echo "${REPO_ROOT}/.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-${task_key}.json"
  else
    echo "${REPO_ROOT}/.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json"  # 向后兼容
  fi
}
...
| **会话状态** | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json` | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-planner-refactor.json` |
|  | 单例 ❌ 覆盖 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-todo-enhance.json` ✅ 隔离 |
| **卡片状态** | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json` | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/planner-refactor/task-runner-state.json` |
|  | 单例 ❌ 覆盖 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/todo-enhance/task-runner-state.json` ✅ 隔离 |
...
cat workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json | jq '.task_key'
# 预期："planner-refactor"
...
# 3. 验证任务A的状态未丢失
cat workdocs/任务拆解/2026-03-03_planner-refactor/contracts/_active_task.json | jq '.task_key'
# 预期："planner-refactor"
...
# 现有状态：.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json
...
mv .artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json .artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-legacy-task.json
```
...
- [ ] 状态文件：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state-<task_key>.json`
...
| `workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json` | 活跃任务索引 | - |
| `workdocs/任务拆解/<task_key>/contracts/_active_task.json` | 任务级配置 | - |
| `workdocs/任务拆解/<task_key>/contracts/vk_cards.json` | 卡片契约 | - |
...
3. 全局状态冲突：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json` 被多个任务覆盖
...
ls -la .worktrees/
ls -la .artifacts/states/task_splits/<task_split_dir>/<task_key>/
...
3. **会话状态冲突**（类型3a）：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json` 被多个任务覆盖
