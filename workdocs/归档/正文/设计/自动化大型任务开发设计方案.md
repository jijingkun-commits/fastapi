...
> 前置文档：
> - [VK 依赖分析报告](../研究报告/vibe_kanban依赖分析报告.md)（947 行，核心基础）
> - [Heartbeat 替代 Cron 交叉验证研究报告](../研究报告/heartbeat替代cron_交叉验证研究报告.md)（529 行，触发器方案）
> - [VK 方案评审报告](../审查报告/vibe_kanban方案评审报告.md)（183 行，评审反馈）
...
| 术语 | 定义 |
|------|------|
| task-runner-state | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json`，运行时状态文件，记录当前卡片、状态映射、执行模式等 |
...
| bootstrap_kernel | `scripts/coder4/coder4_bootstrap_kernel.py`，coder4 每轮执行的核心调度脚本（463 行） |
| wt-flow.sh | `scripts/coder4/wt-flow.sh`，worktree 生命周期管理脚本（231 行） |
| done_gate | 卡片完成验收门禁，执行 acceptance_checks 并记录证据 |
| attempt | 一次卡片执行尝试的完整记录，包含开始/结束时间、结果、证据、worktree 路径等 |
| scope_guard | `scripts/coder4/coder4_scope_guard.py`，作用域切换守卫，确保 coder4 在正确的任务上下文中执行 |
...
flowchart TD
    ...
    subgraph "状态层（本地 JSON 唯一真理源）"
        ...
        L5["task-runner-state.json::gate_results/merge_results/<card_id>/<br/>执行历史"]
    end
...
# 替换 L231
state_path = ".artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json"
...
parser.add_argument("--state-dir", default=".artifacts/states/task_splits/<task_split_dir>/<task_key>",
                    help="本地状态文件目录")
...
def heartbeat_turn(
    ...
    state_dir: str = ".artifacts/states/task_splits/<task_split_dir>/<task_key>",
...
```bash
# wt-flow.sh next [--state-dir=.artifacts/states/task_splits/<task_split_dir>/<task_key>]
# 读取 task-runner-state.json，推进到下一张可执行卡片

next() {
    local state_dir="${STATE_DIR:-.artifacts/states/task_splits/<task_split_dir>/<task_key>}"
...
```bash
# wt-flow.sh verify <card_id> [--state-dir=.artifacts/states/task_splits/<task_split_dir>/<task_key>]
...
verify() {
    ...
    local task_key=$(jq -r '.task_key' workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json)
    local cards_file="workdocs/任务拆解/${task_key}/contracts/vk_cards.json"
    ...
    if [[ -z "$checks" ]]; then
        ...
        echo '{"passed": true, "evidence": "no_checks_defined"}'  # 写入 task-runner-state.json::gate_results.$card_id
        return 0
    fi
    ...
    # 写入验收结果
    local result_file="$state_dir/task-runner-state.json"  # 内联字段: gate_results.$card_id
...
```bash
# wt-flow.sh list [--state-dir=.artifacts/states/task_splits/<task_split_dir>/<task_key>]
# 输出当前任务队列的表格视图
...
3. wt-flow.sh verify <card_id>
   ...
   ├── 记录证据到 task-runner-state.json::gate_results/merge_results/<card_id>/
   ├── 通过 → card_status = "done"
...
sequenceDiagram
    ...
    participant AT as task-runner-state.json::gate_results/merge_results/
...
| 规则 | 说明 |
|------|------|
| 按 `task_key` 隔离 | attempt 目录按 `task_key` 隔离：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/<task_key>/<card_id>/`，不同任务的同名卡片不会混在一起 |
| 保留最近 N 个 | 每张卡片保留最近 20 个 attempt 文件，超出部分在 `create_attempt()` 时自动清理最旧的文件 |
| 任务完成后归档 | 任务全部 done 后，`task-runner-state.json::gate_results/merge_results/<task_key>/` 目录可整体压缩归档到 `.omc/archive/` |
| 磁盘空间监控 | `heartbeat_turn()` 每轮检查 `.artifacts/states/task_splits/<task_split_dir>/<task_key>/` 目录大小，超过 100MB 时在 Telegram 通知中告警 |
...
| `POST /api/task-attempts/{id}/merge`（触发合并） | 覆盖 | `scripts/coder4/wt-flow.sh merge <card_id>`（本地 git 操作） |
| `DONE_GATE_CHECK` 在 `attempt_repo_dir` 中执行 | 覆盖 | `wt-flow.sh verify` 在 `worktree_path` 中执行 |
| Web UI 查看 attempt 历史 | 不覆盖 | 命令行替代：`ls .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/<card_id>/` |
...
| 查询方式 | 命令 | 输出 |
|---------|------|------|
| 文件级查询 | `ls .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/C01/` | 列出所有 attempt 文件 |
| 详情查询 | `cat .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/C01/attempt_003.json \| jq .` | 单个 attempt 详情 |
| 汇总查询 | `wt-flow.sh list` | 全部卡片状态 + 当前 attempt |
| Telegram 查询 | 用户发送"查看 C01 历史" | coder4 读取 attempts 目录并汇总回复 |
| 台账查询 | `cat .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl \| jq -s .` | 完整执行台账 |
...
| scope request | `.artifacts/states/task_splits/<task_split_dir>/coder4_scope_request.json` | ~10 | 低（含 project_id） | project_id 改为本地解析 |
...
| Rule 27 | attempt 通过 VK API 创建/查询 | **改写**——改为本地 `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/` 目录 |
...
| # | 文件路径 | 行数 | 改造内容 | Phase |
|---|---------|------|---------|-------|
| 1 | `scripts/coder4/coder4_bootstrap_kernel.py` | 463 | L231 VK API 读取改本地；L382/L396 VK API 写入改本地；新增 `--local-mode`；新增 `trigger_next_round()` | P1 |
| 2 | `scripts/coder4/coder4_scope_guard.py` | 260 | L180 `project_id` 改为可选；移除 VK API 依赖 | P1 |
| 3 | `.cursor/scripts/coder4/set_active_task.py` | 141 | `--project-id` 改为可选参数（VK 只读推送时使用） | P1 |
| 4 | `scripts/coder4/wt-flow.sh` | 231 | 新增 next/verify/list 子命令（Ch 6 设计） | P1 |
| 5 | `workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json` | ~20 | `project_id` 标记为可选 | P1 |
...
| 11 | `docs/开发文档/流程与工具/Coder4自动执行总控手册.md` | ~30 | 中改：更新触发器说明（cron → hooks-first）、状态文件路径 | P2 |
| 12 | `docs/开发文档/流程与工具/开发工作流.md` | ~20 | 小改：更新 wt-flow.sh 命令说明、VK 相关流程 | P2 |
| 13 | `docs/开发文档/流程与工具/OpenClaw自动执行故障字典.md` | ~15 | 小改：新增 hooks 相关故障条目 | P2 |
| 14 | `docs/开发文档/流程与工具/VibeKanban多Worktree本机开发测试.md` | ~10 | 小改：标注 VK 降级为只读后的流程变化 | P3 |
| 15 | `workdocs/任务拆解/_templates/parallel_plan_template.md` | ~10 | 小改：移除 VK 同步相关模板字段 | P2 |
...
| 9 | `.artifacts/states/task_splits/<task_split_dir>/coder4_scope_request.json` | ~10 | `project_id` 改为可选 | P1 |
...
| # | 文件路径 | 用途 | Phase |
|---|---------|------|-------|
| 11 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json` | 运行时状态唯一真理源（Ch 4 Schema） | P1 |
| 12 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/<card_id>/attempt_NNN.json` | 本地 attempt 记录（Ch 7 设计） | P1 |
| 13 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl` | 执行台账（从仓外迁移） | P1 |
| 14 | `scripts/coder4/coder4_vk_sync.py` | VK 只读推送脚本（fire-and-forget） | P2 |
...
| 文件权限错误 | **否** | - | - | .artifacts/states/task_splits/<task_split_dir>/<task_key>/ 不可写 |
...
RUN_LOCK = Path(".artifacts/states/task_splits/<task_split_dir>/<task_key>/coder4-run.lock")
IDEMPOTENCY_FILE = Path(".artifacts/states/task_splits/<task_split_dir>/<task_key>/coder4-idempotency.json")
...
| P1.4 | attempt 本地化（`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/`） | attempt 创建/查询/汇总正常 |
...
| 数据边界 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>` 与 `~/.openclaw*` 纳入备份与恢复闭环 | 可在 30 分钟内完成恢复演练 |
...
flowchart TD
    ...
    HA --> BK
    BK --> ST[".artifacts/states/task_splits/<task_split_dir>/<task_key>/*.json"]
    BK --> WT["wt-flow.sh"]
    WT --> CODE["项目代码仓库"]
    BK -.-> VK["VK 只读同步（可选）"]
...
| 路径 | 用途 | 权限建议 | 备注 |
|------|------|---------|------|
| `.artifacts/states/task_splits/<task_split_dir>/<task_key>/` | 运行时状态（state、attempt、ledger） | `700` | 仅执行用户可读写 |
...
```
watchdog 巡检：
1. 读取 .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json
...
|---|-----------|------------|-----------|---------|------|
| P-01 | `_active_task.json` 路径声明 | `AGENTS.md` | §1 真理源路径 | `grep "_active_task" AGENTS.md` | 已迁移 |
| P-02 | `WORKFLOW_AUTO.md` 路径声明 | `AGENTS.md` | §1 真理源路径 | `grep "WORKFLOW_AUTO" AGENTS.md` | 已迁移 |
| P-03 | `VK_AGENT_PROMPTS.md` 路径声明 | `AGENTS.md` | §1 真理源路径 | `grep "VK_AGENT_PROMPTS" AGENTS.md` | 已迁移 |
| P-04 | `task-runner-state.json` 路径声明 | `AGENTS.md` | §1 真理源路径 | `grep "task-runner-state" AGENTS.md` | 已迁移 |
| P-05 | `task-ledger.jsonl` 路径声明 | `AGENTS.md` | §1 真理源路径 | `grep "task-ledger" AGENTS.md` | 已迁移 |

#### B.4.2 硬顺序执行协议（~800 字符 → WORKFLOW_AUTO.md）

| # | 原约束摘要 | 迁移目标文件 | 迁移后位置 | 验证方式 | 状态 |
|---|-----------|------------|-----------|---------|------|
| E-01 | 步骤 1: 读取 `_active_task.json` | `WORKFLOW_AUTO.md` | §执行协议 Rule-NEW-01 | `grep "active_task" WORKFLOW_AUTO.md` | 已迁移 |
| E-02 | 步骤 2: scope guard 校验 | `WORKFLOW_AUTO.md` | §执行协议 Rule-NEW-02 | `grep "scope_guard" WORKFLOW_AUTO.md` | 已迁移 |
| E-03 | 步骤 3: bootstrap_kernel 调用 | `WORKFLOW_AUTO.md` | §执行协议 Rule-NEW-03 | `grep "bootstrap_kernel" WORKFLOW_AUTO.md` | 已迁移 |
| E-04 | 步骤 4: 分支决策（见 B.4.3） | `WORKFLOW_AUTO.md` | §决策矩阵 | 见 B.4.3 逐条验证 | 已迁移 |
| E-05 | 步骤 5: 证据收集与输出 | `WORKFLOW_AUTO.md` | §执行协议 Rule-NEW-05 | `grep "evidence" WORKFLOW_AUTO.md` | 已迁移 |
| E-06 | 步骤 6: 三行结论输出格式 | `VK_AGENT_PROMPTS.md` | §6 输出模板 | `grep "结论" VK_AGENT_PROMPTS.md` | 已迁移 |

#### B.4.3 分支决策矩阵（~1000 字符 → WORKFLOW_AUTO.md）

| # | 原约束摘要 | 迁移目标文件 | 迁移后位置 | 验证方式 | 状态 |
|---|-----------|------------|-----------|---------|------|
| D-01 | action=preflight_blocked 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-01 | A/B 对比：触发 preflight_blocked 场景 | 已迁移 |
| D-02 | action=seed 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-02 | A/B 对比：首次初始化场景 | 已迁移 |
| D-03 | action=activate 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-03 | A/B 对比：激活新卡片场景 | 已迁移 |
| D-04 | action=dispatch/inprogress 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-04 | A/B 对比：执行中卡片场景 | 已迁移 |
| D-05 | action=dispatch/inreview 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-05 | A/B 对比：验收中卡片场景 | 已迁移 |
| D-06 | action=blocked_depends 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-06 | A/B 对比：依赖阻塞场景 | 已迁移 |
| D-07 | action=all_done 分支 | `WORKFLOW_AUTO.md` | §决策矩阵 Branch-07 | A/B 对比：全部完成场景 | 已迁移 |

#### B.4.4 硬约束清单（~600 字符 → AGENTS.md）

| # | 原约束摘要 | 迁移目标文件 | 迁移后位置 | 验证方式 | 状态 |
|---|-----------|------------|-----------|---------|------|
| C-01 | 禁止 manual-db-fallback | `AGENTS.md` | §2 硬约束 | `grep "manual-db-fallback" AGENTS.md` | 已迁移 |
| C-02 | 禁止全量读取（仅读当前卡片相关文件） | `AGENTS.md` | §2 硬约束 | `grep "全量读取" AGENTS.md` | 已迁移 |
| C-03 | 禁止空转（无实质操作时必须输出原因） | `AGENTS.md` | §2 硬约束 | `grep "空转" AGENTS.md` | 已迁移 |
| C-04 | 禁止跳过 scope_guard | `AGENTS.md` | §2 硬约束 | `grep "scope_guard" AGENTS.md` | 已迁移 |
| C-05 | 禁止修改 _active_task.json | `AGENTS.md` | §2 硬约束 | `grep "active_task.*禁止" AGENTS.md` | 已迁移 |
| C-06 | 禁止直接操作 VK API（仅通过 sync 脚本） | `AGENTS.md` | §2 硬约束 | `grep "VK API" AGENTS.md` | 已迁移 |
| C-07 | 禁止修改其他卡片的 worktree | `AGENTS.md` | §2 硬约束 | `grep "worktree.*禁止" AGENTS.md` | 已迁移 |
| C-08 | 单次执行超时上限（timeoutSeconds） | `AGENTS.md` | §2 硬约束 | `grep "timeout" AGENTS.md` | 已迁移 |
| C-09 | 输出必须包含三行结论格式 | `VK_AGENT_PROMPTS.md` | §6 输出模板 | `grep "结论" VK_AGENT_PROMPTS.md` | 已迁移 |
| C-10 | 禁止在 heartbeat 中执行破坏性 git 操作 | `AGENTS.md` | §2 硬约束 | `grep "git.*禁止" AGENTS.md` | 已迁移 |

#### B.4.5 输出格式模板（~300 字符 → VK_AGENT_PROMPTS.md）

| # | 原约束摘要 | 迁移目标文件 | 迁移后位置 | 验证方式 | 状态 |
|---|-----------|------------|-----------|---------|------|
| O-01 | 第一行：结论（NORMAL_DISPATCH / ALL_DONE / BLOCKED 等） | `VK_AGENT_PROMPTS.md` | §6.1 结论行 | A/B 对比输出格式 | 已迁移 |
| O-02 | 第二行：当前动作描述 | `VK_AGENT_PROMPTS.md` | §6.2 动作行 | A/B 对比输出格式 | 已迁移 |
| O-03 | 第三行：证据（commit SHA / test result） | `VK_AGENT_PROMPTS.md` | §6.3 证据行 | A/B 对比输出格式 | 已迁移 |
...
迁移实测记录（2026-02-28）：
- Checklist 状态：P-01~P-05、E-01~E-06、D-01~D-07、C-01~C-10、O-01~O-03 共 31 项均已迁移。
- Watchdog payload：缩减为巡检指令（<500 字符），仅输出 `WATCHDOG_OK|WATCHDOG_STALE|WATCHDOG_NO_STATE`。
- A/B 抽样场景：`preflight_blocked`、`dispatch/inprogress`、`all_done` 三类分支输出均与迁移前语义一致。
...
| 文件 | 路径 | 引用章节 |
|------|------|---------|
| bootstrap_kernel.py | `scripts/coder4/coder4_bootstrap_kernel.py` | Ch 5, Ch 8.5, Ch 9.2, Ch 10.1, 附录 A |
| scope_guard.py | `scripts/coder4/coder4_scope_guard.py` | Ch 8.5, Ch 10.1, 附录 A.2 |
| set_active_task.py | `.cursor/scripts/coder4/set_active_task.py` | Ch 8.5, Ch 10.1, 附录 A.2 |
| wt-flow.sh | `scripts/coder4/wt-flow.sh` | Ch 6, Ch 10.1 |
| _active_task.json | `workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json` | Ch 3.5, Ch 5.2, Ch 8.5 |
| vk_cards.json | `workdocs/任务拆解/.../contracts/vk_cards.json` | Ch 3.5, Ch 4 |
...
| scope_request.json | `.artifacts/states/task_splits/<task_split_dir>/coder4_scope_request.json` | Ch 8.5, Ch 10.2 |
...
| 文档 | 路径 | 行数 | 本方案引用 |
|------|------|------|----------|
| VK 依赖分析报告 | `workdocs/归档/专题/研究报告/vibe_kanban依赖分析报告.md` | 947 | Ch 2-5, Ch 8, Ch 10-13 |
| Heartbeat 交叉验证研究报告 | `workdocs/归档/专题/研究报告/heartbeat替代cron_交叉验证研究报告.md` | 529 | Ch 3, Ch 9, Ch 12 |
| VK 方案评审报告 | `workdocs/归档/报告/审查报告/vibe_kanban方案评审报告.md` | 183 | Ch 4, Ch 11, Ch 12, Ch 14 |
...
> 审查依据：
> - [VK 依赖分析报告](../研究报告/vibe_kanban依赖分析报告.md)（947 行）
> - [Heartbeat 替代 Cron 交叉验证研究报告](../研究报告/heartbeat替代cron_交叉验证研究报告.md)（529 行）
> - [VK 方案评审报告](../审查报告/vibe_kanban方案评审报告.md)（183 行）
