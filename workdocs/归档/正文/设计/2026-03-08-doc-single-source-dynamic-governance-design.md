...
- 过程文档的唯一职责冻结为：记录设计推导、实施拆解、风险与审批证据，落点固定在 `workdocs/设计/`、`workdocs/归档/正文/`、`docs/内部参考/决策记录.md` 等过程与内部参考层。
...
- 边界:
  - 不在本方案内重写全部历史文档。
- 设计过程层统一迁到 `workdocs/设计/` 与 `workdocs/归档/正文/`。
...
- 架构规则调整时，历史原因放进 `workdocs/设计/`、`workdocs/归档/正文/设计/` 或 `内部参考`，主文档只保留冻结后的最终口径。
...
| 过程与决策层 | 记录设计推导、方案冻结、实施计划、审批证据 | `workdocs/设计/`、`workdocs/归档/正文/` |
...
3. 开发者先在主文档原章节融入事实变更。
4. 若存在设计推导、风险论证或实施拆解，再补充到 `workdocs/设计/`、`workdocs/归档/正文/设计/` 或 `内部参考/迭代需求/`。
...
- 设计与过程真相源：`workdocs/设计/*.md`、`workdocs/归档/正文/设计/*.md`、`workdocs/归档/正文/需求/*.md` 与 `workdocs/归档/正文/实施计划/*.md`。
...
| D-02-history-layer-isolation | FR-02 | 需要记录设计推导或实施拆解 | 存在设计/实施证据 | 证据写入 `workdocs/设计/`、`workdocs/归档/正文/设计/` 或 `内部参考` | `history_written_into_main_doc -> fail_fast` | `file`,`doc_role`,`evidence_type` | `process_doc_redirect` | `python3 scripts/docs_guard.py --strict` |
...
| T06 | [T01,T02,T03,T04,T05] | `workdocs/归档/正文/设计/2026-03-08-doc-single-source-dynamic-governance-design.md` | `design_freeze_sections` | add | `python3 scripts/docs_guard.py --strict` |
...
| R02 | 融合迁移时把历史事实删丢 | 主文档内容变短但过程证据缺失 | `workdocs/设计/` + `workdocs/归档/正文/设计/` + `内部参考` | 先把历史事实迁入过程文档，再删除主文档旧增量段；若缺失则回退该文档改动 |
...
clarify_handoff_contract:
  ...
  design_source: workdocs/归档/正文/设计/2026-03-08-doc-single-source-dynamic-governance-design.md
  handoff_ready: true
  required:
    ...
    implementation_seeds:
      ...
      - task_id: T05
        ...
        change_type: modify
      - task_id: T06
        file_paths: [workdocs/归档/正文/设计/2026-03-08-doc-single-source-dynamic-governance-design.md]
