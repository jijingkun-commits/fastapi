...
| B. 仅改 `wt-flow.sh` 命名空间 | 解决分支/worktree/会话冲突，主流程改动集中 | 读侧脚本仍读全局 `.artifacts/states/task_splits/<task_split_dir>/<task_key>`，会出现“写隔离、读错位” | 中 | 中 |
...
- 架构:
  ...
  - 统一资源命名为 task_key 作用域:
    ...
    - 会话状态: `.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json`
    - 卡片状态: `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json`
    - 证据目录: `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/<card_id>/`
...
- 组件:
  ...
  - `scripts/coder4/coder4_bootstrap_kernel.py`
    - 默认状态路径从固定 `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json` 升级为“按 active_task 推导的 task_key 路径”。
...
- [ ] 是否提供一次性迁移脚本，将 legacy `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json` 迁移到 `.artifacts/states/task_splits/<task_split_dir>/<task_key>/`。
