...
> 文档状态：总控主计划（中度拆分版本）
> 创建日期：2026-02-18
> 适用范围：`workdocs/归档/正文/需求/` 及其关联实施文档
...
在进入 `P1/C01` 前，必须先通过 `C00`。

1. 固化四风险修订：`evidence` 门禁分场景、scene 解析入口统一、插件后置、引用锚点修订。
2. 统一取消接口权威口径：`POST /api/v1/chat/runs/{run_id}/cancel`。
3. C00 未完成时，允许继续评审与文档补全，但不得开启实现卡。
4. C00 证据需同步到波次看板与 P1 专题文档，避免只留在分析稿。

执行状态（2026-02-20）：`C00` 已通过，可进入 `P1/C01`。

说明：本状态为镜像信息；执行进度唯一回填入口为 `迁移执行波次_implementation_plan.md`（Gate 看板 + 当前卡执行记录）。
...
2. 实施前置先过 `C00`（四风险修订 + 口径统一 + 证据回填）。
3. 实施阶段按 Wave 顺序执行：P1（运行时可取消）-> P2（工具治理一期）-> P3（Skill 多用户版本治理）-> P4（记忆检索增强）-> P5（稳态增强）-> P6（收口与回滚演练）。
4. Batch-1 ~ Batch-5 继续作为跨专题门禁与依赖管理视图。
5. 最后由 Batch-6 统一收口（回归、回滚演练、终稿归档）。
...
5. **风险五：实现口径分叉（接口路径/行号引用漂移）**
   缓解：取消接口统一使用 `POST /api/v1/chat/runs/{run_id}/cancel`，文档引用优先函数/模块锚点。
...
| Batch-1 | 待开始 | - | 依赖 Batch-0 完成 + C00 通过 |
