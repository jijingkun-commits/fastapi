...
> 日期：2026-02-28
> 对应需求：`workdocs/归档/正文/需求/自动化大型任务开发_主机方案_requirements.md`
> 主设计输入：`workdocs/归档/正文/设计/自动化大型任务开发设计方案.md`
> 执行清单输入：`workdocs/归档/正文/实施计划/自动化大型任务开发_全量打钩板清单.md`
...
| source_id | 来源文件 | 抽取原子 |
|-----------|---------|---------|
| SRC-01 | `workdocs/归档/正文/设计/自动化大型任务开发设计方案.md` | hooks-first、三层状态、P0~P3 路线、Ch15~17 门禁 |
| SRC-02 | `workdocs/归档/正文/实施计划/自动化大型任务开发_全量打钩板清单.md` | Phase 勾选项、Exit Gate、会话交接字段 |
| SRC-03 | `workdocs/归档/专题/研究报告/vibe_kanban依赖分析报告.md` | VK 依赖范围与清理边界 |
| SRC-04 | `workdocs/归档/专题/研究报告/heartbeat替代cron_交叉验证研究报告.md` | wake/agent/cron 触发能力拆分 |
| SRC-05 | `workdocs/归档/报告/审查报告/vibe_kanban方案评审报告.md` | 风险缓解建议与校验项 |

### 0.2 对齐与审批标记

- `DESIGN_APPROVAL_FALLBACK_ACK`：本轮按用户在 2026-02-28 当前会话明确指令执行；
- `SUPERPOWERS_ARTIFACT_UNALIGNED`：未发现 `workdocs/设计/` 下同主题 `design_approved: true` 文档，需在实施前补齐设计归档；
...
| 状态层 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/*` | state/attempt/ledger 收敛 | 禁止引入第四套主状态源 |
...
| `attempt_*` | 单轮执行证据 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/<task_key>/` | 每轮新增，按保留策略清理 |
| `ledger` | 阶段性可审计台账 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl` | 全流程追加，禁止覆盖 |
...
1. `workdocs/归档/正文/需求/自动化大型任务开发_主机方案_requirements.md`
2. `workdocs/归档/正文/实施计划/自动化大型任务开发_主机方案_implementation_plan.md`
3. `workdocs/任务拆解/<YYYY-MM-DD_主题>/...`（由 `/jjk-vkplan` 产出）
...
| P1-04 | attempt/ledger 本地化与清理策略 | `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/`, `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl` | 停止清理并恢复归档快照 |
...
python3 scripts/coder4/coder4_bootstrap_kernel.py \
  ...
  --active-task workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json
```
...
- 代码锚点：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/`、`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl`。
...
```bash
grep -n "待迁移" workdocs/归档/正文/设计/自动化大型任务开发设计方案.md
```
...
implementation_tasks:
  ...
  - task_id: T-04
    ...
    change_type: modify
    acceptance_cmds:
      - python3 scripts/coder4/coder4_bootstrap_kernel.py --local-mode --apply-bootstrap --active-task workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json
  ...
  - task_id: T-06
    ...
    phase: P1
    file_paths:
      - .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/
      - .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl
    symbols:
      - create_attempt
      - record_attempt_evidence
    change_type: add
    acceptance_cmds:
      - test -f .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json || true
      - test -f .artifacts/states/task_splits/<task_split_dir>/<task_key>/task-ledger.jsonl || true
...
planning_contract:
  ...
  cards:
    - card_id: C00
      ...
      acceptance_checks:
        - bash scripts/coder4_external_backup.sh
      evidence_entry: workdocs/归档/正文/实施计划/自动化大型任务开发_全量打钩板清单.md
    ...
    - card_id: C02
      ...
      acceptance_checks:
        - python3 scripts/coder4/coder4_bootstrap_kernel.py --local-mode --active-task workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json
      evidence_entry: workdocs/归档/正文/实施计划/自动化大型任务开发_主机方案_implementation_plan.md
    ...
    - card_id: C06
      ...
      acceptance_checks:
        - python3 scripts/docs_guard.py --strict
      evidence_entry: workdocs/归档/正文/设计/自动化大型任务开发设计方案.md
