...
1. `workdocs/归档/正文/设计/2026-03-03-parallel-task-worktree-namespace-design.md`
2. `workdocs/归档/正文/需求/多任务并行Worktree命名空间与异步收口_requirements.md`
...
- 设计文档：`workdocs/归档/正文/设计/2026-03-03-parallel-task-worktree-namespace-design.md`
...
1. active 索引：`workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json`
2. 任务级状态根：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/`
3. 卡片状态：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json`
4. 会话状态：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json`
5. 证据目录：`.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json::gate_results/merge_results/<card_id>/`
...
|---|---|---|---|---|---|
| P1-01 | C01 | 分支/worktree 命名空间隔离，兼容 legacy | `scripts/coder4/wt-flow.sh::cmd_create` `::_resolve_worktree_path_for_card` | `bash scripts/coder4/wt-flow.sh next --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>`（双任务模拟） | 保留 legacy 分支解析分支 |
| P1-02 | C01 | 会话状态与卡片状态按 task_key 隔离 | `scripts/coder4/wt-flow.sh::_task_state_file` `::_read_state/_save_state` | `bash scripts/coder4/wt-flow.sh list --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>` | 回退到全局 state 文件路径 |
...
| P2-01 | C03 | 读侧脚本默认路径统一到 task_key 状态根 | `scripts/coder4/coder4_bootstrap_kernel.py` `scripts/coder4/coder4_vk_sync.py` `scripts/coder4/check_integration_gate.py` | `python3 scripts/coder4/coder4_bootstrap_kernel.py --local-mode --active-task workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json` | 增加 fallback 到 legacy 路径 |
| P2-02 | C03 | scope_guard already_active 补齐 task_key 判定 | `scripts/coder4/coder4_scope_guard.py` | `python3 scripts/coder4/coder4_scope_guard.py --repo-root /Users/jijingkun/bojxAI/fastapi --task-split-dir <task_split_dir> --scope-request .artifacts/states/task_splits/<task_split_dir>/coder4_scope_request.json` | 回退到 split+project 判定 |
...
| P4-01 | C06 | 文档/命令口径同步，避免旧示例误导 | `.cursor/commands/jjk-cardrun.md` `docs/开发文档/流程与工具/开发工作流.md` `docs/SUMMARY.md` | `python3 scripts/docs_guard.py --strict` | 回退文档改动 |
| G-1 | G01 | 串行闭环与作用域门禁校验 | `scripts/coder4/wt-flow.sh` `scripts/coder4/coder4_scope_guard.py` | `bash scripts/coder4/wt-flow.sh verify C01 --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>` + `bash scripts/coder4/wt-flow.sh merge --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>` | 失败时阻断推进 |
...
_state_root() {
  ...
  [[ -n "$task_key" ]] && echo "${REPO_ROOT}/.artifacts/states/task_splits/<task_split_dir>/<task_key>/${task_key}" || echo "${REPO_ROOT}/.artifacts/states/task_splits/<task_split_dir>/<task_key>"
}
```
...
def resolve_state_file(active_task_path: Path) -> Path:
    ...
    if task_key:
        return Path(".artifacts/states/task_splits/<task_split_dir>/<task_key>") / task_key / "task-runner-state.json"
    return Path(".artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json")
```
...
implementation_tasks:
  - task_id: T-01
    ...
    change_type: modify
    acceptance_cmds:
      - bash scripts/coder4/wt-flow.sh next --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
      - bash scripts/coder4/wt-flow.sh list --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
  ...
  - task_id: T-02
    ...
    acceptance_cmds:
      - bash scripts/coder4/wt-flow.sh list --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
      - bash scripts/coder4/wt-flow.sh verify C01 --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
    rollback_point: 切回全局 `.artifacts/states/task_splits/<task_split_dir>/<task_key>/wt-flow-state.json` 与 `.artifacts/states/task_splits/<task_split_dir>/<task_key>/task-runner-state.json`
  ...
  - task_id: T-04
    ...
    change_type: modify
    acceptance_cmds:
      - python3 scripts/coder4/coder4_bootstrap_kernel.py --local-mode --active-task workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json
      - python3 scripts/coder4/coder4_vk_sync.py --active-task workdocs/任务拆解/<task_split_dir>/contracts/_active_task.json --dry-run --output -
  ...
  - task_id: T-05
    ...
    change_type: modify
    acceptance_cmds:
      - python3 scripts/coder4/coder4_scope_guard.py --repo-root /Users/jijingkun/bojxAI/fastapi --task-split-dir <task_split_dir> --scope-request .artifacts/states/task_splits/<task_split_dir>/coder4_scope_request.json
  ...
  - task_id: T-10
    ...
    acceptance_cmds:
      - bash scripts/coder4/wt-flow.sh verify C01 --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
      - bash scripts/coder4/wt-flow.sh merge --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
  ...
  cards:
    - card_id: C01
      ...
      acceptance_checks:
        - bash scripts/coder4/wt-flow.sh next --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
        - bash scripts/coder4/wt-flow.sh list --state-dir .artifacts/states/task_splits/<task_split_dir>/<task_key>
      evidence_entry: workdocs/归档/正文/实施计划/多任务并行Worktree命名空间与异步收口_implementation_plan.md
...
| WP-D：测试与文档门禁 | AI-4 | `tests/**`、`.cursor/commands/**`、`docs/开发文档/流程与工具/**`、`docs/SUMMARY.md` | 依赖 A/B/C 输出 | 1 天 | 回归用例+文档口径统一+docs_guard 通过 |
