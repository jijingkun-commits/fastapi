...
> 上游设计：`workdocs/归档/设计/2026-03-06-chat-multi-session-concurrency-design.md`
> 对应需求：`workdocs/归档/需求/chat-multi-session-concurrency_requirements.md`
  ...
  cards:
    - card_id: C01
      ...
      - 真实链路 UAT：stop 后立即新开第 4 会话，不得出现 `3/3` 误拦截 --grep MSC-CL-008
        - pnpm --dir web exec playwright test e2e/chat-multi-session-concurrency.spec.cjs
      - 真实链路 UAT：stop 后立即新开第 4 会话，不得出现 `3/3` 误拦截 --grep MSC-CL-002
      evidence_entry: workdocs/归档/实施计划/chat-multi-session-concurrency_implementation_plan.md
    ...
    - card_id: G01
      ...
      acceptance_checks:
        - python3 scripts/check_workflow_contract.py --mode clarify_plan --requirements-path workdocs/归档/需求/chat-multi-session-concurrency_requirements.md --implementation-path workdocs/归档/实施计划/chat-multi-session-concurrency_implementation_plan.md --output workdocs/归档/机读校验/chat-multi-session-concurrency_clarify_plan_alignment.json
        - python3 scripts/check_workflow_contract.py --mode planning_temporal_gate --implementation-path workdocs/归档/实施计划/chat-multi-session-concurrency_implementation_plan.md --output workdocs/归档/机读校验/chat-multi-session-concurrency_planning_temporal_gate.json
...
3. 若实现阶段发现设计缺口，应回到 `workdocs/归档/设计/2026-03-06-chat-multi-session-concurrency-design.md` 先修 design，再回补本计划。
