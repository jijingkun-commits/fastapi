...
> 上游设计：`workdocs/归档/设计/2026-03-08-doc-single-source-dynamic-governance-design.md`
...
- 过程文档层：`workdocs/设计/*.md`、`workdocs/归档/设计/*.md`、`docs/内部参考/迭代需求/*.md`
- 规则与门禁：`.cursor/rules/doc_sync.mdc`、`scripts/docs_guard.py`、`scripts/check_doc_sync.sh`
- 治理手册与索引：`docs/开发文档/流程与工具/开发工作流.md`、`docs/SUMMARY.md`

### 1.3 非范围

- 不在本轮一次性清洗所有历史主文档。
- 不废弃 `docs/内部参考/迭代需求/` 的残余历史入口，但新的设计过程统一进入 `workdocs/设计/`。
...
requirements_contract:
  ...
  clarify_handoff_source: workdocs/归档/设计/2026-03-08-doc-single-source-dynamic-governance-design.md#clarify_handoff_contract
...
fr_contract_matrix:
  ...
  - fr_id: FR-02
    ...
    input_contract:
      ...
      source_of_truth: workdocs/设计/*.md + workdocs/归档/设计/*.md + docs/内部参考/迭代需求/*.md
    output_contract:
...
traceability_matrix:
  - design_item: D-01-main-doc-current-state-only
    ...
    evidence_entry: workdocs/归档/实施计划/文档单一真相源与动态融合治理_implementation_plan.md
  ...
  - design_item: D-08-plan-alignment-gate
    ...
    task_id: T06
    tc_id: TC-DSG-07
    acceptance_cmd_ref: 先执行 `bash scripts/repo_python.sh` 获取解释器，再执行 `<PYTHON_BIN> scripts/check_workflow_contract.py --mode clarify_plan --requirements-path workdocs/归档/需求/文档单一真相源与动态融合治理_requirements.md --implementation-path workdocs/归档/实施计划/文档单一真相源与动态融合治理_implementation_plan.md --output workdocs/归档/机读校验/文档单一真相源与动态融合治理_clarify_plan_alignment.json`
    evidence_entry: workdocs/归档/实施计划/文档单一真相源与动态融合治理_implementation_plan.md
```
