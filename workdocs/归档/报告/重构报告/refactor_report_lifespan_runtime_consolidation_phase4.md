...
- followup_tasks:
  - graph provider 外移已在 `workdocs/归档/报告/重构报告/refactor_report_lifespan_runtime_consolidation_phase4_closeout.md` 收口，不再单列新阶段。
  - 若未来全面引入 request 级注入，再把深层 service/tool 从 getter 过渡到 `request.app.state.runtime` 显式取依赖。
- refactor_report_path: `workdocs/归档/报告/重构报告/refactor_report_lifespan_runtime_consolidation_phase4.md`
