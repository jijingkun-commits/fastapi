...
| 术语 | 定义 |
|------|------|
| task-runner-state | `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json`，运行时状态文件，记录当前卡片、状态映射、执行模式等 |
...
# 替换 L231
state_path = "docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json"
...
parser.add_argument("--state-dir", default="docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>",
                    help="本地状态文件目录")
...
def heartbeat_turn(
    ...
    state_dir: str = "docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>",
...
```bash
# wt-flow.sh next [--state-dir=docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>]
# 读取 task-runner-state.json，推进到下一张可执行卡片

next() {
    local state_dir="${STATE_DIR:-docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>}"
...
```bash
# wt-flow.sh verify <card_id> [--state-dir=docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>]
...
verify() {
    ...
    local task_key=$(jq -r '.task_key' docs/内部参考/任务拆解/<task_split_dir>/_active_task.json)
...
```bash
# wt-flow.sh list [--state-dir=docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>]
# 输出当前任务队列的表格视图
...
| 规则 | 说明 |
|------|------|
| 按 `task_key` 隔离 | attempt 目录按 `task_key` 隔离：`docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/<task_key>/<card_id>/`，不同任务的同名卡片不会混在一起 |
...
| 磁盘空间监控 | `heartbeat_turn()` 每轮检查 `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/` 目录大小，超过 100MB 时在 Telegram 通知中告警 |
...
| Web UI 查看 attempt 历史 | 不覆盖 | 命令行替代：`ls docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/<card_id>/` |
...
| 查询方式 | 命令 | 输出 |
|---------|------|------|
| 文件级查询 | `ls docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/C01/` | 列出所有 attempt 文件 |
| 详情查询 | `cat docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/C01/attempt_003.json \| jq .` | 单个 attempt 详情 |
| 汇总查询 | `wt-flow.sh list` | 全部卡片状态 + 当前 attempt |
| Telegram 查询 | 用户发送"查看 C01 历史" | coder4 读取 attempts 目录并汇总回复 |
| 台账查询 | `cat docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-ledger.jsonl \| jq -s .` | 完整执行台账 |
...
| scope request | `docs/内部参考/任务拆解/<task_split_dir>/.state/coder4_scope_request.json` | ~10 | 低（含 project_id） | project_id 改为本地解析 |
...
| Rule 27 | attempt 通过 VK API 创建/查询 | **改写**——改为本地 `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/` 目录 |
...
| 5 | `docs/内部参考/任务拆解/<task_split_dir>/_active_task.json` | ~20 | `project_id` 标记为可选 | P1 |
...
| 9 | `docs/内部参考/任务拆解/<task_split_dir>/.state/coder4_scope_request.json` | ~10 | `project_id` 改为可选 | P1 |
...
| # | 文件路径 | 用途 | Phase |
|---|---------|------|-------|
| 11 | `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json` | 运行时状态唯一真理源（Ch 4 Schema） | P1 |
| 12 | `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/<card_id>/attempt_NNN.json` | 本地 attempt 记录（Ch 7 设计） | P1 |
| 13 | `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-ledger.jsonl` | 执行台账（从仓外迁移） | P1 |
...
| 文件权限错误 | **否** | - | - | docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/ 不可写 |
...
RUN_LOCK = Path("docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/coder4-run.lock")
IDEMPOTENCY_FILE = Path("docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/coder4-idempotency.json")
...
| P1.4 | attempt 本地化（`docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json::gate_results/merge_results/`） | attempt 创建/查询/汇总正常 |
...
| 数据边界 | `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>` 与 `~/.openclaw*` 纳入备份与恢复闭环 | 可在 30 分钟内完成恢复演练 |
...
flowchart TD
    ...
    HA --> BK
    BK --> ST["docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/*.json"]
    BK --> WT["wt-flow.sh"]
    WT --> CODE["项目代码仓库"]
    BK -.-> VK["VK 只读同步（可选）"]
...
| 路径 | 用途 | 权限建议 | 备注 |
|------|------|---------|------|
| `docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/` | 运行时状态（state、attempt、ledger） | `700` | 仅执行用户可读写 |
...
```
watchdog 巡检：
1. 读取 docs/内部参考/任务拆解/<task_split_dir>/.state/<task_key>/task-runner-state.json
...
| _active_task.json | `docs/内部参考/任务拆解/<task_split_dir>/_active_task.json` | Ch 3.5, Ch 5.2, Ch 8.5 |
...
| scope_request.json | `docs/内部参考/任务拆解/<task_split_dir>/.state/coder4_scope_request.json` | Ch 8.5, Ch 10.2 |
...
| 文档 | 路径 | 行数 | 本方案引用 |
|------|------|------|----------|
| VK 依赖分析报告 | `workdocs/归档/研究报告/vibe_kanban依赖分析报告.md` | 947 | Ch 2-5, Ch 8, Ch 10-13 |
| Heartbeat 交叉验证研究报告 | `workdocs/归档/研究报告/heartbeat替代cron_交叉验证研究报告.md` | 529 | Ch 3, Ch 9, Ch 12 |
| VK 方案评审报告 | `workdocs/归档/审查报告/vibe_kanban方案评审报告.md` | 183 | Ch 4, Ch 11, Ch 12, Ch 14 |
