...
> 更新时间：2026-03-10
> 设计状态：`approved`
> 关联方案：`workdocs/归档/设计/2026-03-08-doc-single-source-dynamic-governance-design.md`
...
- 历史设计目录已迁到 `workdocs/归档/设计/`；`docs/内部参考/迭代需求/`、`docs/内部参考/任务拆解/` 当前仍保留为迁移期旧过程路径；`task_split` 下的 `_active_task.json`、`vk_cards.json`、`preflight_status.json`、`consumption_report.json` 等机器契约/过程报告 JSON 暂不强迁。
...
- 新增主题不再同时出现在 `workdocs/设计/`、`docs/内部参考/迭代需求/`、稳定文档三个位置表达“当前态”。
...
2. 历史设计与 `docs/内部参考/迭代需求/` 不再双轨并存；设计过程统一归到 `workdocs/设计/`，历史设计正文统一归档到 `workdocs/归档/设计/`。
