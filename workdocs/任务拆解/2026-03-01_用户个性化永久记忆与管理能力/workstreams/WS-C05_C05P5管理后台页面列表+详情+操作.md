# WS-C05 C05 P5 管理后台页面（列表+详情+操作）

> WS 编号: WS-C05  
> 对应卡片: `C05`  
> 类型: `parallel`  
> 对应 `feature_id`: `P5-01,P5-02`
...
- 新增 /admin/memory 页面与 AdminSidebar 菜单
- 列表筛选、详情抽屉、调试面板统一复用 memory-admin-api SDK
- 治理动作（归档/删除/重建）具备明确反馈与错误提示

### 1.1 功能机制（必填）

- 触发条件: 前置依赖 `C01, C02` 完成
...
- 代码锚点（函数/类级）:
  - `web/src/app/admin/memory/page.tsx::MemoryAdminPage`
  - `web/src/components/admin/MemoryAdminPanel.tsx::MemoryAdminPanel`
  - `web/src/components/admin/memory/MemoryDetailDrawer.tsx::MemoryDetailDrawer`
  - `web/src/lib/memory-admin-api.ts::listMemories`
- 最小样例（可伪代码）:
...
- 最小测试集:
- `cd /Users/jijingkun/.codex/worktrees/978e/fastapi/web && npm run lint`
- 验收标准:
- 后台页面可完成查询与治理闭环
- 前端 lint 通过
...
- 对应 implementation plan `done_gate`: `后台页面可完成查询与治理闭环; 前端 lint 通过`
- 本 WS 负责的门禁子项: `C05:P5-01,P5-02`
...
- 回滚点:
- 移除 AdminSidebar memory 菜单
- 下线 /admin/memory 页面路由
...
card_export:
  id: WS-C05
  feature_id: P5-01,P5-02
  card_key: PP-20260301-USER-MEMORY-ADMIN::WS-C05
  title: C05 P5 管理后台页面（列表+详情+操作）
  type: parallel
