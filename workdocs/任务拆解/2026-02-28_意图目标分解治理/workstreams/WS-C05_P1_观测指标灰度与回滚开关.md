...
- 本包目标:
- 建立 shadow 对账指标与模式切换开关
- 支持灰度放量并可快速回切 heuristic_only
- 输出差异率与 fallback 命中率观测
- 完成定义（DoD）:
...
- 触发条件: 前置依赖 `C04` 已完成并满足 done gate
...
card_export:
  id: WS-C05
  feature_id: P1-06
  card_key: PP-20260228-INTENT-DECOMPOSITION-DB::WS-C05
  title: C05 P1 观测指标灰度与回滚开关
  type: parallel
  ...
  soft_depends_on: []
  depends_on: ["C04"]
  file_whitelist: ["app/services/config_resolver.py", "app/ai/workflow/multi_agent_graph.py", "tests/integration/test_intent_shadow_metrics.py"]
  readonly_scope: []
  owner_fields: []
  ...
  example_refs: ["docs/内部参考/迭代需求/意图目标分解治理_implementation_plan.md#4.6"]
  ...
  done_gate: ["shadow metrics tests green", "rollback switch validated"]
```
