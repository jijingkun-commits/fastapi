...
- 本包目标:
- 控制面仅处理命令/队列/权限，不直接改写语义目标
- planner 主判定输出结构化 intent_plan，形成语义单一真相源
- 保留旧路径兼容字段，先做契约治理再切执行路径
- 完成定义（DoD）:
...
- `app/ai/workflow/multi_agent_graph.py::_infer_model_intent_plan`
...
card_export:
  ...
  card_key: PP-20260228-INTENT-DECOMPOSITION-DB::WS-C01
  title: C01 P1 控制面与语义面分层契约落地
  type: parallel
  ...
  soft_depends_on: []
  depends_on: []
  file_whitelist: ["app/ai/workflow/multi_agent_graph.py", "app/services/chat_service.py", "app/ai/prompts/agent_prompts.py", "tests/unit/test_intent_plan_model_primary.py"]
  readonly_scope: []
  owner_fields: []
  ...
  example_refs: ["workdocs/归档/实施计划/意图目标分解治理_implementation_plan.md#4.1", "workdocs/归档/实施计划/意图目标分解治理_implementation_plan.md#4.2"]
  ...
  done_gate: ["intent layer boundary tests green", "model primary planner tests green"]
```
