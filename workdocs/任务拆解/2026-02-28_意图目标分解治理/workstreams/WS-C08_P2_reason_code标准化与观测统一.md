# WS-C08 C08 P2 reason_code标准化与观测统一
...
- 本包目标:
- fallback_meta.reason 输出稳定 reason_code 枚举
- SSE/日志口径统一，支持按 reason_code 聚合
- 为灰度与运维定位提供统一观测维度
- 完成定义（DoD）:
...
- 触发条件: 前置依赖 `C07` 已完成并满足 done gate
...
- 回滚点:
- `PLANNER_REASON_CODE_VERBOSE=false`
...
card_export:
  id: WS-C08
  feature_id: P2-05
  card_key: PP-20260228-INTENT-DECOMPOSITION-DB::WS-C08
  title: C08 P2 reason_code标准化与观测统一
  type: parallel
  task_mode: implementation-card
  merge_required: true
  execution_mode: serial
  lane: lane-backend-observe
  hard_depends_on: ["C07"]
  soft_depends_on: []
  depends_on: ["C07"]
  file_whitelist: ["app/ai/workflow/multi_agent_graph.py", "app/services/chat_service.py", "tests/unit/test_planner_reason_codes.py"]
  readonly_scope: []
  owner_fields: []
  ...
  example_refs: ["workdocs/归档/实施计划/意图目标分解治理_implementation_plan.md#12.2", "workdocs/归档/实施计划/意图目标分解治理_implementation_plan.md#12.4"]
  ...
  done_gate: ["reason_code normalization pass"]
```
