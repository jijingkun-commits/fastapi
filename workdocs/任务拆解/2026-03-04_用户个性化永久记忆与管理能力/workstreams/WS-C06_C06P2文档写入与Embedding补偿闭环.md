...
- 本包目标：canonical_text 进入 document/chunk 两表
- 完成定义（DoD）：canonical_text 进入 document/chunk 两表; chunk embedding_status=pending 走补偿链路
...
- 代码锚点：
  - `app/services/document_memory_service.py::flush_canonical_memory`
  - `app/services/document_memory_embedding_service.py::compensate_pending_embeddings`
...
- 对应 implementation plan `done_gate`：`workdocs/归档/正文/实施计划/用户个性化永久记忆与管理能力_implementation_plan.md` 中 `C06` 段。
