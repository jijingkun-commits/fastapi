...
- 本包目标：reverse_intent+可定位槽位才归档
- 完成定义（DoD）：reverse_intent+可定位槽位才归档; 敏感信息命中直接拒绝沉淀
...
- 代码锚点：
  - `app/services/memory_sensitive_guard_service.py::detect`
  - `app/services/memory_intent_llm_service.py::apply_reverse_intent`
...
- 对应 implementation plan `done_gate`：`workdocs/归档/正文/实施计划/用户个性化永久记忆与管理能力_implementation_plan.md` 中 `C05` 段。
