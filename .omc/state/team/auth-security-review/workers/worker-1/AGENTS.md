...
```bash
mkdir -p $(dirname .omc/state/team/auth-security-review/workers/worker-1/.ready) && touch .omc/state/team/auth-security-review/workers/worker-1/.ready
```
...
- **Worker**: worker-1
- **Agent Type**: codex
- **Environment**: OMC_TEAM_WORKER=auth-security-review/worker-1
...
- **Inbox**: Read .omc/state/team/auth-security-review/workers/worker-1/inbox.md for new instructions
- **Heartbeat**: Update .omc/state/team/auth-security-review/workers/worker-1/heartbeat.json every few minutes:
  ```json
...
- Path: .omc/state/team/auth-security-review/workers/worker-1/done.json
...
2. Write an ACK file: .omc/state/team/auth-security-review/workers/worker-1/shutdown-ack.json
3. Exit
