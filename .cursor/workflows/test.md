...
1.  **UI/API 层**:
    *   验证响应状态码及 JSON 结构。
    *   如果涉及前端，使用 Playwright 进行 E2E 测试。
2.  **数据层 (Deep verify)**:
...
> **重要**: 优先使用项目内置 Playwright 测试，而非 IDE 内置 Browser MCP 工具。

**可用命令** (在 `web/` 目录下执行):

| 命令 | 用途 |
|------|------|
| `npm test` | 无界面运行所有测试 (CI/CD 推荐) |
| `npm run test:ui` | 可视化 UI 界面选择测试 |
| `npm run test:headed` | 有浏览器窗口运行 |
| `npm run test:debug` | 逐步调试模式 |

**运行单个测试**:
```bash
cd web
npx playwright test chat.spec.cjs      # 聊天功能
npx playwright test todo.spec.cjs      # 待办功能
```

**测试文件位置**: `web/e2e/*.spec.cjs`

**何时使用 IDE Browser MCP**: 仅用于手动探索性测试或快速验证单个交互，不适合批量回归测试。
