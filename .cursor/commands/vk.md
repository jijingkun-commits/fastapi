---
description: VK 看板生成：基于 /vkplan 产物生成建卡内容与导入提示词（默认 strict，严格读取 card_export）
---

# VK 工作流 (VK Workflow)

用于把 `/vkplan` 的拆解结果转成“可直接导入 Vibe Kanban 的卡片内容 + 提示词”。
...
*使用 `/vk` 触发。适合把 `/vkplan` 结果转成看板导入内容。*
