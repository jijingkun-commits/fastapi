---
description: Team 封装命令：执行 /prompts:jjk-imp-ws（自动同步源命令）
---
...
将 `/jjk-imp-ws` 封装为 Team 入口，且始终以源命令文档为唯一真理源。
...
```text
/jjk-team-imp-ws workers=5 role=executor mode=ralph
任务：<在这里写你的任务目标>
```

## 执行协议（强制）

1. 先读取源文件：`.cursor/commands/jjk-imp-ws.md`，严格沿用其约束与产物要求。
2. 以 Team 方式组织执行，并显式执行：`/prompts:jjk-imp-ws`。
3. 禁止把 `/jjk-imp-ws` 当普通文本描述。
...
当 `.cursor/commands/jjk-imp-ws.md` 变更后，重新执行同步脚本即可自动刷新本文件，并同步到：

1. `.claude/commands/jjk-team-imp-ws.md`（CC 端）
2. `~/.codex/prompts/jjk-team-imp-ws.md`（Codex 端）
